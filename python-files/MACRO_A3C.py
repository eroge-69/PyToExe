...
def choose_file_dialog(title="Sélectionnez un fichier", filetypes=(("All files", "*.*"),)):
    root = tk.Tk()
...
def extract_xml_fields(xml_path):
    ...
    # Version normalisée (pour comparaison)
...
def extract_html_fields(html_path, xml_repere_clean=None):
    ...
    if total_td < 11:
        print(f"Attention : le document HTML contient moins de 11 balises <td> (trouvé {total_td}).", file=sys.stderr)

    # 2. Récupérer le chiffre de la 6ᵉ <td>
    page_num_digit = ""
    idx6 = 6 - 1
    if idx6 < total_td:
        td6 = td_list[idx6]
        ...
        if val:
            # Extraire le premier caractère s'il est chiffre
        ...
        print("Avertissement : 6ᵉ <td> non trouvée pour extraire le chiffre.", file=sys.stderr)

    # Extraire et nettoyer le texte d’un <td> donné
    def get_td_text_by_index(idx_1based, remove_leading_digit=False):
        zero_idx = idx_1based - 1
        if zero_idx < 0 or zero_idx >= total_td:
            print(f"Avertissement : <td> numéro {idx_1based} non trouvé (total {total_td}).", file=sys.stderr)
            return ""
        td = td_list[zero_idx]
        # Supprimer les <br>
        ...
        # Nettoyer retours à la ligne, tabulations
        text = text.replace("\n", "").replace("\r", "").replace("\t", "")
        # Si on demande de supprimer le premier caractère s'il est chiffre
        if remove_leading_digit and text and text[0].isdigit():
            text = text[1:]
        return text

    # 3. Extraction des différents td : td2, td1, td10, td11
    # td2 : idx 2, et on supprime le 1er caractère si c'est chiffre
    ...
    # 4. concaténation et minuscules, suppression des espaces
...
# Parcours des fichiers HTML dans le dossier
    ...
    if "fetch('/mouse_position?x=' + x + '&y=' + y);" in content:
         print(f"✅ Script déjà inséré")
    else:
        if '</body>' in content:
            content = content.replace('</body>', script)
            with open(filepath, 'w', encoding='utf-8') as file:
                file.write(content)
            print(f"✅ Script inséré")
        else:
            print(f"⚠️ Pas de </body> trouvé")
...
class MyHttpRequestHandler(http.server.SimpleHTTPRequestHandler):
    def do_GET(self):
        ...
        if self.path.startswith('/mouse_position'):
            ...
            super().do_GET()  # Gère les fichiers HTML, images, CSS, etc.
...
def find_and_move_to_all_images(image_path_1,image_path_2,image_path_3,image_path_4, threshold_distance, html_body_path, output_folder):
    ...
    # Recherche de toutes les occurrences de l'image à l'écran
    locations = []
    try:
    ...
    if locations:
        ...
        print("nombre trouvé : " + str(nombre_case))
        #print("est passé")
        #print(liste_angle)
        #time.sleep(1)
        time.sleep(1)
        ...
        if locations:
            for location in locations:
                ...
                if int(y_cursor_live) > int(derniere_valeur_obtenu_y):
                    ...
                    liste_x.append(x_cursor_live)
                    liste_y.append(y_cursor_live)
        #print("trouvé en plus : " + str(trouve_en_plus))
        ...
        print("nombre de cases détectées " + str(len(liste_x)))
        ...
        for i in range(0, len(liste_x)):
            ...
            if point_a_remove is None:
                print("Pas de box à associer pour le point", i)
                continue
            test = 0
            if point_a_remove in coordinates_des_box:
                ...
                test = 1
            if test == 0:
                print("le point x = " + point_a_remove[0] + " et y = " + point_a_remove[1] + " n'a pas été retiré")
        print("compteur a compté qu'on a enlevé : " + str(compteur_rep) + " élements !")
...
def extract_xml_text(xml_path):
    ...
    if not calques_list:
        print("Aucune balise <Calques> trouvée dans le XML.")
        return ""
    ...
    # 4. Boucle sur InstanceGabarit (inchangé)
...
def comparer_frequences(texte1, texte2, liste_probleme_html=None, liste_probleme_xml=None):
    ...
    if freq1 == freq2:
        print("\nLes deux textes contiennent exactement les mêmes caractères avec les mêmes fréquences.")
        messages.append("IDENTICAL")
        return "\n".join(messages)
    else:
        print("\nDifférences trouvÃ©es dans les fréquences de caractères :")
        messages.append("DIFFERENT")

        # Caractères en plus dans texte1 (et donc en moins dans texte2)
        diff1 = freq1 - freq2
        if diff1:
            print("\nCaractères en plus dans le texte 1 (et donc en moins dans le texte 2) :")
            for char, count in diff1.items():
                print(f"  '{char}' : {count} times")

        # Caractères en plus dans texte2 (et donc en moins dans texte1)
        diff2 = freq2 - freq1
        if diff2:
            print("\nCaractères en plus dans le texte 2 (et donc en moins dans le texte 1) :")
            for char, count in diff2.items():
                print(f"  '{char}' : {count} times")


        # Ajout des phrases problématiques
...
def comparaison(text_file_path, xml_path, concatene_html, concatene_xml):
    ...
    # Vérification extraction TXT
    if not phrase_html_propre or all(s.strip() == "" for s in phrase_html_propre):
        print("Aucun texte extrait du fichier TXT !")
    else:
        print(f"{len(phrase_html_propre)} éléments extraits du fichier TXT.")

    # Récupération phrase XML
    phrase_xml = []
    phrase_xml_propre = []
    ...
    for elem in last_calques.iter('TexteSimple'):
        ...
        # Si <Texte> enfant trouvé, on ne garde que sa valeur
        if texte_enfant is not None:
            valeur = texte_enfant.get('valeur')
            if valeur and len(valeur) > 1 and valeur.lower() not in ("start", "end"):
    ...
    # Comparaison sur la version nettoyée (concaténée)
    liste_probleme_html = []
    ...
    print("\n==== CONCATENE_HTML (TXT nettoyé) ====")
    print(concatene_html)
    print("\n==== CONCATENE_XML (XML nettoyé) ====")
    print(concatene_xml)



    print("problème situé dans le html aux phrases suivantes : ")
    print(liste_probleme_html)
    print("problème situé aux phrases xml suivantes : ")
...
def analyse_pdf_letter_overlap(pdf_path):
    ...
    print(f"DEBUG extract_letter_characteristics pour {pdf_path} : {len(text_attributes)} lettres trouvées")
    if not text_attributes:
        print(f"Aucune lettre trouvée dans {pdf_path}")
        return []

    ancien_rect_comp = text_attributes[0]["bbox"]
    k = 0
    rectangle_pas_bon = []
    mots_superposes = []  # <-- Liste des mots/lettres superposés
    ...
    for attr in text_attributes:
        ...
        for attr_compr in text_attributes:
            ...
            if k > 5 and l > 5 and (not(rectangle in rectangle_pas_bon) or not(rectangle_comp in rectangle_pas_bon)):
                # Récupère les lettres superposées dans les deux contextes
                ...
                # Ajoute les deux mots/lettres superposés à la liste
...
def batch_compare_text_xml_pdf(output_folder):
    ...
    print(f"{len(text_files)} fichiers texte trouvés.")
...
def detect_html_paths_for_xml(folder, xml_filename):
    ...
    # Recherche du HTML TOP (toujours nettoyé, sans suffixe)
    ...
    for full_path in find_all_files_with_extension(folder, ('.html', '.htm')):
        ...
        if "body" in name_clean:
            # Cas du body "par défaut" (pas de suffixe ou suffixe "1")
...
def diff_strict(a: str, b: str, label: str, orig_a: str = None, orig_b: str = None) -> str:
    def get_word_containing_index(text: str, index: int) -> tuple:
        """
        Retourne le mot contenant l'index donné et l'indice du mot dans la liste.
        """
        words = text.split()
        current_pos = 0
    ...
    # Si tout est identique jusqu'au plus court, mais longueurs différentes
...
def map_index_to_original(norm_text, orig_text, diff_index):
    """
    Trouve la position dans le texte original correspondant à l'index diff_index
    dans le texte normalisé (espaces supprimés).
    """
    count = 0
    for orig_idx, char in enumerate(orig_text):
        if char != " ":
            if count == diff_index:
                return orig_idx
            count += 1
    return -1  # Pas trouvé (ne devrait pas arriver)
...
def main():
    ...
    # Sélection du fichier macro .xlsm
    ...
    # Sélection du fichier spec .xlsx
    ...
    # Recherche récursive de toutes les images .png contenant 'image_carre' dans leur nom
    carre_images = [
        str(p) for p in Path(output_folder).rglob("*.png")
        if "image_carre" in p.name
    ]
    if len(carre_images) != 4:
        print(f"Erreur : {len(carre_images)} image(s) 'image_carre*.png' trouvée(s). Il en faut exactement 4.")
        return
    ...
    # Changer de répertoire de travail si nécessaire
    os.chdir(output_folder)

    pairs = batch_compare_text_xml_pdf(output_folder)

    # Recherche de tous les fichiers XML dans le dossier
    #xml_files = [os.path.join(output_folder, f) for f in os.listdir(output_folder) if f.lower().endswith('.xml')]
    xml_files = find_all_files_with_extension(output_folder, ('.xml',))
    if not xml_files:
        print("Aucun fichier XML trouvé dans le dossier. Fin du script.")
        return

    # Pour chaque XML, tenter d'extraire l'identifiant et associer les HTML correspondants
    for text_path, xml_path in pairs:
        print(f"Traitement du fichier TXT: {os.path.basename(text_path)} et XML: {os.path.basename(xml_path)}")
        ...
        if xml_processed is None or not xml_repere_clean:
            print(f"Erreur lors du traitement de {xml_path} ou identifiant vide. Passage au suivant.")
            continue

        # Détecter automatiquement les HTML top et body
        html_top_path, html_body_path = detect_html_paths_for_xml(output_folder, os.path.basename(xml_path))


        if html_top_path:
            print(f"Fichier HTML top détecté: {os.path.basename(html_top_path)}")
            ...
            if result:
                ...
                html_processed = "".join([td2_text, td1_text, td10_text, td11_text])
                print(f"Texte extrait et traité depuis le HTML top: {html_processed}")
                print(f"____________________________________________________________")
                print(f"Texte extrait et traité depuis le XML pour le top: {xml_processed}")
                # --- hack “identique en mémoire” : on complète html_processed si besoin ---
                if xml_repere_clean and xml_repere_clean[-1].isdigit():
                    ...
                    # on reconstruit html_processed en mémoire
                    html_processed = "".join([td2_text, td1_mod, td10_text, td11_text])

                # --- comparaison finale via diff_strict ---
                ...
                print(diff_html_top_xml)
                print("OOOOOOOOOOOOOOOOOOO" + html_processed_original)
                print("OOOOOOOOOOOOOOOOOOO" + html_processed)
                print("OOOOOOOOOOOOOOOOOOO" + norm_html_top)



            else:
                print(f"Erreur lors de l'extraction du HTML top: {html_top_path}")
        else:
            print("Aucun fichier HTML top automatique trouvé pour cet identifiant.")

        if html_body_path:
            print(f"Fichier HTML body détecté: {os.path.basename(html_body_path)}")
            nom_base = os.path.basename(html_body_path).split("-body")[0]
            # Insérer script et lancer serveur, rendre en PDF, annotation graphique
            ...
            time.sleep(5)
            relative_path = os.path.relpath(html_body_path, output_folder)
            relative_url = relative_path.replace("\\", "/").replace(" ", "%20")
            relative_path = os.path.relpath(html_body_path, output_folder)
            relative_url = relative_path.replace("\\", "/").replace(" ", "%20")
            render_html_to_pdf(f"http://localhost:8000/{relative_url}")

            # === APPEL DE LA DÉTECTION DES CASES ===
            ...
            print("DEBUG mots_superposes_texte pour", html_body_path, ":", mots_superposes_texte)
            ...
            # === ÉCRITURE DU RAPPORT POUR CE DOCUMENT ===
            ...
            print("Aucun fichier HTML body automatique trouvé pour cet identifiant.")


    # Après traitement des paires XML/HTML, on peut lancer le batch sur TXT/XML/PDF
    print("Lancement du batch de comparaison texte/XML/PDF pour tout le dossier...")
    #batch_compare_text_xml_pdf(output_folder)
    ...
    print("Lancement de l'extraction HTML vers Excel pour tout le dossier...")
    batch_extract_html_to_excel(output_folder, macro_file_path, spec_file_path)

    print("Traitement terminé.")
    root = tk.Tk()
    messagebox.showinfo("End of generation", "Generation completed successfully !")
