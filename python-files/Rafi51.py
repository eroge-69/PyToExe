...
class EJuiceCalculator(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Rafi Test 5")
        ...
        tabs.addTab(build_tab, "Build EXE")
        
        # Build instructions
        build_label = QLabel("Click the button below to create an executable EXE file. This will allow you to run the program without Python installed.")
        build_label.setWordWrap(True)
        build_layout.addWidget(build_label)
        
        build_btn = QPushButton("Build EXE File", objectName="buildButton")
        build_btn.clicked.connect(self.build_exe)
        build_layout.addWidget(build_btn)
        
        build_status = QLabel("Status: Ready to build")
        ...
        self.recipe_name = QLineEdit("My Custom Liquid")
        ...
        self.nic_strength.setRange(0, 1000)
    ...
    def build_exe(self):
        try:
            self.build_status.setText("Status: Building... (This may take a few minutes)")
            ...
            cmd = [
                ...
                '--clean',
                '--noconfirm',
                os.path.basename(__file__)  # Use current file name
            ]
            
            result = subprocess.run(cmd, capture_output=True, text=True)
            
            if result.returncode == 0:
                self.build_status.setText("Status: Build successful! Check the 'dist' folder.")
                QMessageBox.information(self, "Build Complete", 
                                      "EXE file created successfully!\n\n"
                                      "You can find it in the 'dist' folder.\n"
                                      "This file can be run on any Windows computer without Python installed.")
            else:
                self.build_status.setText("Status: Build failed")
                QMessageBox.critical(self, "Build Failed", 
                                   f"Error building EXE file:\n{result.stderr}")
                
        except Exception as e:
            self.build_status.setText("Status: Build error")
            QMessageBox.critical(self, "Build Error", f"An error occurred: {str(e)}")
