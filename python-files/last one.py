...
class TestHelperApp(QWidget):
    def __init__(self):
        ...
        self.setGeometry(100, 100, 700, 610)  # Increased width for better layout
        ...
        self.setStyleSheet(self.get_dark_stylesheet())  # APPLY DARK MODE
    ...
    def eventFilter(self, obj, event):
        if event.type() == QEvent.Wheel:
            if event.modifiers() == Qt.MiddleButton:
                scrollbar = self.sender().parentWidget()
                if scrollbar:
                    delta = event.angleDelta().y()
                    scrollbar.verticalScrollBar().setValue(scrollbar.verticalScrollBar().value() - delta)
                return True
    ...
    def update_timer_from_input(self):
        self.time_remaining = (self.hour_input.value() * 3600 +
