    ...
    class DummyAndroid:
        def __getattr__(self, name):
            def method(*args, **kwargs):
                print(f"\n      \x1b[38;5;102mAppel fictif à : {name} avec des arguments {args} et des paramètres nommés {kwargs}")
                return None
            return method
    ad = DummyAndroid() 
...
AUTHOR = f'🜲LORENC' if os.name == 'nt' else [f'🜲LORENC', f'༺ LORENC ༻   '][datetime.datetime.now().second % 2]
...
def set_country(country) -> str:
    country_mapping = { 'US': '🇺🇸 SHTETET E BASHKUARA', 'NO': '🇳🇴 NORVEGJIA', 'SE': '🇸🇪 SUEDIA', 'HU': '🇭🇺 HUNGARI', 'FI': '🇫🇮 FINLANDE', 'FR': '🇫🇷 France', 'DE': '🇩🇪 GJERMANI', 'BG': '🇧🇬 BULLGARIA', 'UA': '🇺🇦 Ukraina', 'BR': '🇧🇷 BRAZIL', 'PL': '🇵🇱 POLONI', 'AF': '🇦🇫 Afganistan', 'AL': '🇦🇱 SHQIPERI', 'DZ': '🇩🇿 AlGJERI', 'AO': '🇦🇴 Angola', 'AR': '🇦🇷 ARGJENTINE', 'AM': '🇦🇲 Armenia', 'AU': '🇦🇺 Australia', 'AT': '🇦🇹 Austria', 'AZ': '🇦🇿 AZERBAXHAN', 'BS': '🇧🇸 BAHAMAS', 'BH': '🇧🇭 Bahrajn', 'BD': '🇧🇩 BangladesH', 'BB': '🇧🇧 Barbados', 'BY': '🇧🇾 BJELLORUSIA', 'BE': '🇧🇪 BELGJIKE', 'BZ': '🇧🇿 Belize', 'BJ': '🇧🇯 Benin', 'BT': '🇧🇹 Bhutan', 'BO': '🇧🇴 Boliwia', 'BA': '🇧🇦 Bośnia i Hercegowina', 'BW': '🇧🇼 Botswana', 'CA': '🇨🇦 Kanada', 'CV': '🇨🇻 Wyspy Zielonego Przylądka', 'KH': '🇰🇭 Kambodża', 'CL': '🇨🇱 Chile', 'CM': '🇨🇲 Kamerun', 'CN': '🇨🇳 KINA', 'CO': '🇨🇴 Kolumbia', 'CG': '🇨🇬 Kongo-Brazzaville', 'CD': '🇨🇩 Kongo-Kinszasa', 'CR': '🇨🇷 Kostaryka', 'HR': '🇭🇷 Chorwacja', 'CU': '🇨🇺 Kuba', 'CY': '🇨🇾 Cypr', 'CZ': '🇨🇿 Czechy', 'DK': '🇩🇰 Dania', 'DJ': '🇩🇯 Dżibuti', 'DO': '🇩🇴 Dominikana', 'EC': '🇪🇨 Ekwador', 'EG': '🇪🇬 Egipt', 'SV': '🇸🇻 Salwador', 'EE': '🇪🇪 Estonia', 'ET': '🇪🇹 Etiopia', 'GH': '🇬🇭 Ghana', 'GE': '🇬🇪 Georgia', 'GR': '🇬🇷 Greqia', 'HK': '🇭🇰 Hongkong', 'IS': '🇮🇸 Islandia', 'IN': '🇮🇳 Indie', 'ID': '🇮🇩 Indonezja', 'IR': '🇮🇷 Iran', 'IE': '🇮🇪 Irlandia', 'IL': '🇮🇱 Izrael', 'IT': '🇮🇹 ITALIA', 'JP': '🇯🇵 Japonia', 'JO': '🇯🇴 Jordania', 'KZ': '🇰🇿 Kazachstan', 'KE': '🇰🇪 Kenia', 'KR': '🇰🇷 Korea', 'LV': '🇱🇻 Łotwa', 'LB': '🇱🇧 Liban', 'LT': '🇱🇹 Litwa', 'LU': '🇱🇺 Luksemburg', 'MY': '🇲🇾 Malezja', 'MV': '🇲🇻 MALDIVE', 'MT': '🇲🇹 Malta', 'MX': '🇲🇽 Meksyk', 'MD': '🇲🇩 Mołdawia', 'MC': '🇲🇨 Monako', 'MN': '🇲🇳 Mongolia', 'MA': '🇲🇦 Maroko', 'NP': '🇳🇵 Nepal', 'NL': '🇳🇱 Holandia', 'NZ': '🇳🇿 Nowa Zelandia', 'NI': '🇳🇮 Nikaragua', 'NG': '🇳🇬 Nigeria', 'MK': '🇲🇰 Macedonia VERIUT', 'PK': '🇵🇰 Pakistan', 'PA': '🇵🇦 Panama', 'PY': '🇵🇾 Paragwaj', 'PE': '🇵🇪 Peru', 'PH': '🇵🇭 Filipiny', 'PT': '🇵🇹 Portugalia', 'RO': '🇷🇴 Rumunia', 'RU': '🇷🇺 Rosja', 'SA': '🇸🇦 Arabia Saudyjska', 'SN': '🇸🇳 Senegal', 'RS': '🇷🇸 Serbia', 'SG': '🇸🇬 Singapor', 'SK': '🇸🇰 Słowacja', 'SI': '🇸🇮 Słowenia', 'ZA': '🇿🇦 Republika South AFRIKA', 'ES': '🇪🇸 SPANJA', 'LK': '🇱🇰 Sri Lanka', 'SD': '🇸🇩 Sudan', 'SR': '🇸🇷 Surinam', 'CH': '🇨🇭 Szwajcaria', 'SY': '🇸🇾 Syria', 'TW': '🇹🇼 Tajwan', 'TJ': '🇹🇯 Tadżykistan', 'TZ': '🇹🇿 Tanzania', 'TH': '🇹🇭 Tajlandia', 'TG': '🇹🇬 Togo', 'TT': '🇹🇹 Trynidad i Tobago', 'TN': '🇹🇳 Tunezja', 'TR': '🇹🇷 Turqia', 'TM': '🇹🇲 Turkmenistan', 'UG': '🇺🇬 Uganda', 'AE': '🇦🇪 EMIRATET BASHKUARA ARABE', 'GB': '🇬🇧 BRITANIA E MADHE', 'UY': '🇺🇾 Urugwaj', 'UZ': '🇺🇿 Uzbekistan', 'VE': '🇻🇪 Wenezuela', 'VN': '🇻🇳 Wietnam', 'YE': '🇾🇪 Jemen', 'ZM': '🇿🇲 Zambia', 'ZW': '🇿🇼 Zimbabwe', 'Uɴᴋɴᴏᴡɴ': '🇦🇱 PIRATI SHQIPTAR' }; result = f'{country_mapping.get(country, "🇦🇱 PIRATI SHQIPTAR")} [{country}]'
    return rwsa(result)
...
{GREEN} ≼ ⟬ {GRAY} Exemple  = {GOLD}{nickn}{GREEN} ⟭ ≽  {BLUE}      

name = {RESET}""") or nickn
intro=f"""
{GOLD}Portal par défaut: (portal.php)
...
{GREEN} ≼ ⟬ {GRAY}Opsioni i rekomanduar N° = {GOLD} 8{GREEN} ⟭ ≽  {BLUE} 
...
""")
    ...
    try:
        ...
        new_output_file = os.path.join(folder_path, f"PORTAL_MAC-LORENC✓ᵘʳᶫˢᶜᵃᶰ·ᶦᵒ·{count_str}_{datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.txt")
        os.rename(output_file_path, new_output_file)
        print(f'\n{GOLD}Fichier téléchargé avec {len(result_urls)} nouveaux portails et enregistré : {new_output_file}')
        time.sleep(4)
...
{GREEN} ≼ ⟬ {GRAY} your choice is= {GOLD}{portal_input}{GREEN} ⟭ ≽  {BLUE}
...
{GREEN} ≼ ⟬ {GRAY} You can type Multi Portal, and end with an empty line {GREEN} ≼ ⟬ {GRAY} {GOLD}(2x Enter) {GRAY}:
""")
...
{GREEN} ≼ ⟬ {GRAY} Number of files in COMBO directory= {GOLD}{str(say)}{GREEN} ⟭ ≽  {BLUE} 

Choose the file number from the list""")
 	dsyno=str(input(f"\nNuméro de fichier = {RESET}")) or str(say)
 	say=0
 	if "1"=="1":
	 	for files in os.listdir(dir):
 			say=say+1
...
#{GREEN} ≼ ⟬ {GRAY}Exemple = {GOLD}0{GREEN} ⟭ ≽  {BLUE} 

#wybór = {RESET}''') or '0'

    say=0
    if dsyno=="0":
        print(f"\n\n\n{GREEN}  ⟬ {GRAY}Préfixes MAC Combo{GREEN} ⟭   {BLUE} \n")
	...
	{GREEN} ≼ ⟬ {GRAY}Exemple = {GOLD}20{GREEN} ⟭ ≽  {BLUE} 
...
{GREEN} ≼ ⟬ {GRAY}Exemple = {GOLD}100000{GREEN} ⟭ ≽  {BLUE} 
...
#################
    ...
    {GREEN}⟬ {GRAY}Listes MAC Combo{GREEN} ⟭   {BLUE}
""")
        for i, file in enumerate(files): say=say+1; print(f"    {GRAY} ⍟ {GOLD}{str(i + 1)}{GRAY}: {GREEN}{file}{GRAY} ⍟    ")
        file_choice = int(input(f"""
    
    {GREEN} ≼ ⟬ {GRAY}Number of files in directory COMBO = {GOLD}{str(say)}{GREEN} ⟭ ≽ {BLUE}
...
File number = {RESET}""") or say) - 1
    ...
    {GREEN} ≼ ⟬ {GRAY} Votre choix = {GOLD}{dosyaa}{GREEN} ⟭ ≽  {BLUE}
...
{GREEN} ≼ ⟬ {GRAY}Exemple = {GOLD}{sanitized_url}{GREEN} ⟭ ≽  {BLUE} 
...
{GREEN} ≼ ⟬ {GRAY} Votre choix = {GOLD}{Dosyab}{GREEN} ⟭ ≽  {BLUE}
...
def hit(mac,trh,panel,real,m3ulink,durum,vpn,livelist,vodlist,serieslist,playerapi,SN,SNENC,SNCUT,DEV,DEVENC,SG,SING,SINGENC,kanalsayisi,filmsayisi,dizisayisi,adult):
    ...
    try:
        imza=f"""⚜️▓ᴄʀᴀᴄᴋɪɴɢ ɪꜱ ɴᴏᴛ ᴀ ᴄʀɪᴍᴇ ɪꜱ ᴀʀᴛ▓⚜️
🎲🇦🇱══🇦🇱🆂🆃🅱 🆃🅸🆃🅰🅽🅸🆄🅼🇦🇱══🇦🇱🎲 
🎲🇦🇱●👑🅺🅸🅽🅶 🅸🅿🆃🆅 🅰🅻🅱🅰🅽🅸🅰👑
🎲🇦🇱●SHEFI AL➤👑🇦🇱🅻🅾🆁🅴🅽🅲 🅺🅾🅻🅰🇦🇱👑
🎲🇦🇱●LIDHJA➤ """+str(real)+"""
🎲🇦🇱●PANELI➤ http://"""+str(panel)+"""/c/"""+str(playerapi)+"""
🎲🇦🇱●TIPI KERKIMIT➤ """+uzmanm+"""
🎲🇦🇱●MAKINA➤ """+str(mac)+"""
🎲🇦🇱●DATA E MBARIMIT➤ """+str(trh)+"""
🎲🇦🇱●DATA E KERKIMIT➤ """+str(time.strftime('%d-%m-%Y'))+"""/"""+str(time.strftime('%H:%M:%S'))+"""
🎲🇦🇱●HITE NGA➤ """+str(nickn)+"""
🎲🇦🇱●VPN INFO ⬇️
🎲🇦🇱●VPN GJENDJA➤ """+str(durum)+"""
🎲🇦🇱●SHTETI VPN➤ """+str(country)+"""
🎲🇦🇱●🎲🇦🇱══🇦🇱🆂🆃🅱 🆃🅸🆃🅰🅽🅸🆄🅼🇦🇱══🇦🇱🎲       

🎲🇦🇱●INFO E BOXIT ⬇️
🎲🇦🇱●KODI Adult➤ """+str(adult)+"""
🎲🇦🇱●Sᴇʀɪᴀʟ➤ """+str(SNENC)+""" 
🎲🇦🇱●Sᴇʀɪᴀʟ SHKURT➤ """+str(SNCUT)+"""
🎲🇦🇱●🖥️PAISJA ID1➤ """+str(DEVENC)+"""
🎲🇦🇱●🖥️PAISJA ID2➤ """+str(SINGENC)+"""
🎲🇦🇱●🎲🇦🇱══🇦🇱🆂🆃🅱 🆃🅸🆃🅰🅽🅸🆄🅼🇦🇱══🇦🇱🎲
🎲🇦🇱●LIDHJA ᴍ3ᴜ➤ """+str(m3ulink)+"""  
🎲🇦🇱●➤🇽🇰🇮🇹🇬🇷🇾🇪🇿🇲🇿🇼🇺🇾🇻🇮🇺🇿🇻🇺🇻🇦🇻🇪🇻🇳🇲🇰🇫🇷 
...
"""
    ...
    except:set_title_name(f"X n'a pas enregistré le hit X | ({hityaz}) | LORENC Scaner | AL")
...
{RESET} 
╭──► {GOLD}LORENC IPTV KERKIM   {RESET} 
│
│{GRAY} {'PANELI►':<8} {INVERT}{str(panel)[:20]}{RESET} 
│{GRAY} {'MAC►':<8} """+tokenr+str(mac)+f"""  {RESET}
│
│{GRAY} {'GJENDJA►':<8} """ + status_code + f"""  {RESET} 
│
│{GRAY} {'TE GJITHA►':<8} {CYAN}"""+str(total)+f""" {GRAY}• """+f"""{FRED}%"""+str(oran)+f""" {GRAY}•{GOLD} """+str(cpm)+f"""ᴄᴘᴍ  {RESET}
│
│{GRAY} {'HITET►':<8} {GREEN}PO {GRAY}("""+FYELLOW+str(hityaz)+GRAY+""") • ("""+str(hitr)+str(hitc)+GRAY+f""") {BLUE}JO  {RESET}
│{GRAY} {'ᴠᴘɴ►':<8} {GREEN}PO {GRAY}("""+GOLD+str(lduruno)+GRAY+f""") • ("""+GOLD+str(lduruon)+GRAY+f""") {RED}JO  {RESET}
│
│{GRAY} {'DATA ORA AKTUALE►':<10} {GOLD}{san_current_time_date}  {RESET}
│{GRAY} {'KOHA NE PUNE►':<10} {GOLD}{run_time}  {RESET}
│
│{GRAY} {'SHTETI ᴠᴘɴ►':<10} {GOLD}{country}  {RESET}
│
╰─► {GOLD}ALBANIA {DARKRED}{knight_rider(f' NE KERKIM...  ')} ALBANIA    {RESET} 
""")
	...
	try:
		...
		if 'active_cons' in veri:
			...
			if bitism=="null":
				...
				mt=("""
🎲🇦🇱●➤INFO XTREAM ⬇️
🎲🇦🇱●MESAZHI I PANELIT➤ """+str(message)+""" 
🎲🇦🇱●PANELI➤ http://"""+panel+"""/c/
🎲🇦🇱●LIDHJA➤ http://"""+realm+""":"""+port+"""/c/
🎲🇦🇱●PORTA➤ """+port+"""
🎲🇦🇱●PERDORUESI➤ """+userm+"""
🎲🇦🇱●KODI➤ """+pasm+"""
🎲🇦🇱●SKADON ME➤ """+bitism+""" 
🎲🇦🇱●LIDHJE AKTIVE➤ """+acon+"""
🎲🇦🇱●LIDHJE MAKSIMALE➤ """+mcon+""" 
🎲🇦🇱●GJENDJA➤ """+status+"""
🎲🇦🇱●ZONA KOHORE➤ """+timezone+f""" 
🎲🇦🇱●➤👑🅺🅸🅽🅶 🅸🅿🆃🆅 🅰🅻🅱🅰🅽🅸🅰👑 """)
	except:pass
	return mt
...
#    timeout, xbagx, xbag, xbag1 = 15, 10, 12, 4
    ...
    panelsay=panelsay+1
    bots=bots+1
    for mc in range(botsay,macuz,4): # à l'origine --> 4):
        get_run_time()
        total=mc
        if dsyno=="0":
            mac=randommac()
