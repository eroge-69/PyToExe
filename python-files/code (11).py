# notapro.py (versão com sublinhado corrigido e independente)
...
class NotaPro(tk.Tk):
    ...
    def _create_menu(self):
        ...
        file_menu.add_command(label="Novo", accelerator="Ctrl+N", command=self._new_file); file_menu.add_command(label="Abrir...", accelerator="Ctrl+O", command=self._open_file); file_menu.add_command(label="Salvar", accelerator="Ctrl+S", command=self._save_file); file_menu.add_command(label="Salvar Como...", command=self._save_as_file); file_menu.add_separator(); file_menu.add_command(label="Sair", command=self._exit_app)
        edit_menu = tk.Menu(self.menu_bar, tearoff=0); self.menu_bar.add_cascade(label="Editar", menu=edit_menu)
        edit_menu.add_command(label="Desfazer", accelerator="Ctrl+Z", command=self.text_widget.edit_undo); edit_menu.add_command(label="Refazer", accelerator="Ctrl+Y", command=self.text_widget.edit_redo); edit_menu.add_separator(); edit_menu.add_command(label="Recortar", accelerator="Ctrl+X", command=lambda: self.text_widget.event_generate("<<Cut>>")); edit_menu.add_command(label="Copiar", accelerator="Ctrl+C", command=lambda: self.text_widget.event_generate("<<Copy>>")); edit_menu.add_command(label="Colar", accelerator="Ctrl+V", command=lambda: self.text_widget.event_generate("<<Paste>>")); edit_menu.add_separator(); edit_menu.add_command(label="Selecionar Tudo", accelerator="Ctrl+A", command=self._select_all); edit_menu.add_command(label="Data/Hora", accelerator="F5", command=self._insert_datetime)
        format_menu = tk.Menu(self.menu_bar, tearoff=0); self.menu_bar.add_cascade(label="Formatar", menu=format_menu)
        self.wrap_var = tk.BooleanVar(value=True); format_menu.add_checkbutton(label="Quebra Automática de Linha", onvalue=True, offvalue=False, variable=self.wrap_var, command=self._toggle_word_wrap); format_menu.add_separator(); format_menu.add_command(label="Alterar Cor do Trecho...", command=self._change_selection_color); format_menu.add_command(label="Grifar trecho...", command=self._highlight_selection); format_menu.add_command(label="Sublinhar trecho...", accelerator="Ctrl+U", command=self._apply_custom_underline); format_menu.add_separator(); format_menu.add_command(label="Cor do Texto Padrão...", command=self._change_default_text_color); format_menu.add_command(label="Cor de Fundo...", command=self._change_bg_color)
        view_menu = tk.Menu(self.menu_bar, tearoff=0); self.menu_bar.add_cascade(label="Exibir", menu=view_menu)
        zoom_menu = tk.Menu(view_menu, tearoff=0); view_menu.add_cascade(label="Zoom", menu=zoom_menu); zoom_menu.add_command(label="Aumentar", accelerator="Ctrl++", command=lambda: self._zoom(10)); zoom_menu.add_command(label="Diminuir", accelerator="Ctrl+-", command=lambda: self._zoom(-10)); zoom_menu.add_command(label="Restaurar Zoom Padrão", accelerator="Ctrl+0", command=lambda: self._zoom(0)); view_menu.add_separator(); self.status_bar_var = tk.BooleanVar(value=True); view_menu.add_checkbutton(label="Barra de Status", onvalue=True, offvalue=False, variable=self.status_bar_var, command=self._toggle_status_bar)

    def _bind_shortcuts(self):
        self.bind("<Control-n>", lambda e: self._new_file()); self.bind("<Control-o>", lambda e: self._open_file()); self.bind("<Control-s>", lambda e: self._save_file()); self.bind("<Control-a>", lambda e: self._select_all()); self.bind("<F5>", lambda e: self._insert_datetime()); self.bind("<Control-plus>", lambda e: self._zoom(10)); self.bind("<Control-minus>", lambda e: self._zoom(-10)); self.bind("<Control-0>", lambda e: self._zoom(0)); self.bind("<Control-y>", lambda e: self.text_widget.edit_redo()); self.bind("<Control-u>", lambda e: self._apply_custom_underline()); self.text_widget.bind("<MouseWheel>", self._zoom_on_scroll)
    ...
    def _save_npro_file(self, filepath):
        ...
        for tag_name in self.text_widget.tag_names():
            if tag_name.startswith("style_"):
                ...
                props_to_check = {"foreground": "", "background": "", "borderwidth": "", "relief": "", "borderbackground": ""}
                for prop in props_to_check:
                    value = self.text_widget.tag_cget(tag_name, prop)
                    if value: config[prop] = value # Apenas adiciona se não for vazio/padrão
    ...
    def _load_npro_file(self, filepath):
        ...
        for tag_info in data.get("tags", []):
            ...
            config_to_apply = tag_info.get("config", {})
    ...
    def _highlight_selection(self):
        if not self.text_widget.tag_ranges(tk.SEL):
            messagebox.showinfo("Nenhuma Seleção", "Selecione um trecho de texto para grifar.")
            return
        ...
        tag_name = f"style_{self.style_counter}"; self.style_counter += 1
        self.text_widget.tag_add(tag_name, "sel.first", "sel.last")
        self.text_widget.tag_config(tag_name, background=blended_color)
        self._on_text_change(None)

    # --- FUNÇÃO DE SUBLINHADO CORRIGIDA ---
    def _apply_custom_underline(self):
        if not self.text_widget.tag_ranges(tk.SEL):
            messagebox.showinfo("Nenhuma Seleção", "Selecione um trecho de texto para sublinhar.")
            return
        ...
        tag_name = f"style_{self.style_counter}"; self.style_counter += 1
        self.text_widget.tag_add(tag_name, "sel.first", "sel.last")
        # Aplica a cor escolhida como uma borda inferior, simulando um sublinhado colorido
        self.text_widget.tag_config(
            tag_name,
            borderwidth=[0, 0, 0, 1],
            relief="solid",
            borderbackground=chosen_color[1]
        )
        self._on_text_change(None)
