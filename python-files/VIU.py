...
# Function to prompt the user for input URL
def get_input_url():
    # Create the main window (hidden)
    root = tk.Tk()
    root.withdraw()  # Hide the main window (we only need the input box)
    
    # Display an input dialog
    url = simpledialog.askstring("Input", "Enter the URL:", parent=root)
    
    # If the user clicked Cancel, url will be None
    if url:
        return url
    else:
...
# Function to download subtitle using yt
...
# Function to process M3U8 URL and download the video using yt
...
def main():
    ...
    if sub_data:
        ...
        # Use current directory for input and output folder
        current_directory = os.getcwd()
        input_folder = os.path.join(current_directory, "Temp")
        output_folder = os.path.join(current_directory, "VIU_DL")
