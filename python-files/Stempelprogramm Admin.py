...
class StempelApp:
    def __init__(self, root):
        ...
        if os.path.exists(ICON_DATEI):
            try:
                bild = Image.open(ICON_DATEI).resize((470, 118))
        ...
        for col, w in zip(["Projekt","Aktion","Zeit","Pause","Arbeitszeit"], [160,120,160,120,120]):
            self.tree.heading(col, text=col, command=lambda c=col: self.sortiere_spalte(c, False))
            self.tree.column(col, width=w)
        self.tree.pack(fill="both", expand=True, padx=10, pady=5)

        # Laufende Einträge farblich hervorheben
        self.tree.tag_configure("laufend", background="#2a4f7c", foreground="white")
        ...
        self.menu.add_command(label="Projekt umbenennen", command=self.projekt_umbenennen)
    ...
    def projekt_umbenennen(self):
        alt = self.auswahl_projekt.get()
        neu = simpledialog.askstring("Projekt umbenennen", f"Neuer Name für Projekt '{alt}':", initialvalue=alt)
        if neu and neu != alt:
            # In Projektliste ersetzen
            self.projekte = [neu if p == alt else p for p in self.projekte]
            speichere_daten(PROJEKT_DATEI, self.projekte)
            self.dropdown["values"] = self.projekte
            self.auswahl_projekt.set(neu)

            # In Stempelungen ersetzen
    ...
    def lade_tree(self):
        ...
        for ein in self.stempelzeiten:
            ...
            if ein["ausstempel"]:
                zeit += " → " + ein["ausstempel"]
            aktion = "Laufend" if ein["ausstempel"] is None else "Fertig"

            iid = self.tree.insert("", "end", values=(
                ein["projekt"], aktion, zeit,
                ein["pause_min"] if ein["pause_min"] is not None else "",
                ein["arbeitszeit_h"] if ein["arbeitszeit_h"] is not None else ""))

            if aktion == "Laufend":
                self.tree.item(iid, tags=("laufend",))

    # --- Sortierfunktion ---
    def sortiere_spalte(self, spalte, rueckwaerts=False):
        daten = [(self.tree.set(k, spalte), k) for k in self.tree.get_children("")]
        try:
            daten.sort(key=lambda t: float(t[0]) if t[0] != "" else -1, reverse=rueckwaerts)
        except ValueError:
            daten.sort(key=lambda t: t[0], reverse=rueckwaerts)

        for index, (val, k) in enumerate(daten):
            self.tree.move(k, "", index)

        self.tree.heading(spalte, command=lambda: self.sortiere_spalte(spalte, not rueckwaerts))
    ...
    def exportieren(self):
        ...
        with open("stempel_export.csv", "w", newline="", encoding="utf-8") as f:
            ...
            for ein in self.stempelzeiten:
                ...
                if ein["ausstempel"]:
                    zeit += " → " + ein["ausstempel"]
                writer.writerow([ein["projekt"], "Laufend" if ein["ausstempel"] is None else "Fertig",
                                 zeit,
