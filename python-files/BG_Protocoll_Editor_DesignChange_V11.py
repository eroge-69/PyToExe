...
def setup_modern_flat_style(root, base_size=10):
    ...
    font_italic = ("Segoe UI", base_size, "italic")
    ...
    style.configure("Subtle.TLabel", background=CARD_COLOR, foreground=SUBTLE_TEXT_COLOR, font=font_italic)
    ...
    style.configure("ActiveRow.TLabel", background=ACTIVE_GREEN_BG, foreground=TEXT_COLOR)
...
class ProtocolEditor:
    ...
    def __init__(self, root):
        ...
        self.total_time_var = tk.StringVar(value="Total Time: 00:00")
        self.step_total_time_vars = [tk.StringVar(value="Step Time: 00:00") for _ in range(8)]
    ...
    def _create_file_toolbar(self):
        ...
        total_time_label = ttk.Label(frame_file, textvariable=self.total_time_var, font=("Segoe UI", 10, "bold"))
        total_time_label.pack(side='right', padx=PAD_MD)
    ...
    def _create_step_content(self, parent, step_idx, headers, vcmd):
        ...
        ttk.Entry(top_bar, textvariable=name_var, width=20).pack(side='left')
        ttk.Label(top_bar, textvariable=self.step_total_time_vars[step_idx], style="Subtle.TLabel").pack(side='left', padx=(PAD_SM, PAD_MD))
        ...
        for i in range(8):
            ...
            for col, field in enumerate(columns_order, start=2):
                if field == 'label':
                    ...
                    var.trace_add("write", lambda *args, s_idx=step_idx: self._on_duration_change(s_idx))
                    row_widgets[field] = var
            ...
            self._on_valve_change(global_idx, row_widgets)

    def _on_duration_change(self, step_idx, *args):
        self._update_step_time(step_idx)
        self._update_total_time()

    def _update_step_time(self, step_idx):
        total_seconds = 0.0
        start_index = step_idx * 8
        for i in range(8):
            try:
                total_seconds += self.rows_widgets[start_index + i]['dwell'].get()
            except (tk.TclError, KeyError): pass
        minutes = int(total_seconds // 60)
        seconds = int(total_seconds % 60)
        self.step_total_time_vars[step_idx].set(f"Step Time: {minutes:02d}:{seconds:02d}")

    def _update_total_time(self):
        total_seconds = 0.0
        for row in self.rows_widgets:
            try: total_seconds += row['dwell'].get()
            except (tk.TclError, KeyError): pass 
        minutes = int(total_seconds // 60)
        seconds = int(total_seconds % 60)
        self.total_time_var.set(f"Total Time: {minutes:02d}:{seconds:02d}")

    def _update_all_times(self):
        for i in range(8): self._update_step_time(i)
        self._update_total_time()

    def _on_valve_change(self, row_index, widgets):
        if 'reagent_label' not in widgets: return
        valve_value = widgets['active_valve'].get()
        new_style = "ActiveRow.TLabel" if valve_value != "None" else "OddRow.TLabel" if (row_index % 2) != 0 else "EvenRow.TLabel"
    ...
    def clear_all(self):
        ...
        self._update_all_times()
        if hasattr(self, '_update_tab_titles'): self._update_tab_titles()
    ...
    def open_file(self):
        ...
        if header_line.startswith(">R2*"):
            remainder = header_line[4:].strip("*")
            if remainder.startswith("M"):
                parts = remainder.split("_", 1)
                method_code = parts[0]
                filename_part = parts[1].rsplit(".txt", 1)[0] if len(parts) > 1 else ""
                try:
                    number = int(method_code[1:])
                    for name, num in METHOD_MAPPING.items():
                        if num == number: self.selected_method.set(name); break
                except (ValueError, IndexError): pass
                self.custom_filename.set(filename_part)
        line_iter = iter(lines)
        for line in line_iter:
            if line.startswith("*CONS*"):
                cons_values = [int(x) for x in line.split(";")[1:]]
                for i in range(5): self.cons_vars[i].set(cons_values[i])
            elif line.startswith("*ROWS*"): break
        for row_idx in range(64):
            try:
                parts = [int(x) for x in next(line_iter).split(";")]
                active_bits = parts[1:8]
                if sum(active_bits) == 1: self.rows_widgets[row_idx]['active_valve'].set(f"V{active_bits.index(1) + 1}")
                else: self.rows_widgets[row_idx]['active_valve'].set("None")
                fields_order = ['pre_fill', 'tray_func', 'heater', 'tray_pos', 'fill_p', 'vacuum_p', 'dwell', 'min_t', 'max_t', 'bubble', 'calib', 'waste_p', 'heater_p', 'label']
                for j, field in enumerate(fields_order):
                    val = parts[8 + j]
                    if field in ['fill_p', 'vacuum_p', 'heater_p']: self.rows_widgets[row_idx][field].set(str(val))
                    elif field in ['dwell', 'min_t', 'max_t']: self.rows_widgets[row_idx][field].set(float(val) / 10.0)
                    elif field == 'label':
                        if (row_idx % 8) == 0:
                            step_idx = row_idx // 8
                            self.step_label_vars[step_idx].set(self.label_num_to_text.get(val, "No Selection"))
                    else: self.rows_widgets[row_idx][field].set(val)
            except StopIteration: break
        self._update_all_times()
        if hasattr(self, '_update_tab_titles'): self._update_tab_titles()

    def save_file(self):
        method_number = METHOD_MAPPING.get(self.selected_method.get(), 1)
        filename_part = self.custom_filename.get().strip() or "Untitled"
        header_line = f">R2*M{method_number}_{filename_part}.txt*"
        default_filename = f"M{method_number}_{filename_part}.txt"
        ...
        try:
            with open(filepath, 'w') as f:
                ...
                cons_line = '*CONS*;' + ';'.join(str(v.get()) for v in self.cons_vars)
                f.write(cons_line + '\n')
                f.write('*ROWS*;64\n')
                for idx, row in enumerate(self.rows_widgets):
                    step_idx = idx // 8
                    label_num = self.label_text_to_num.get(self.step_label_vars[step_idx].get(), 0)
                    valve_bits = [0] * 7
                    val = row['active_valve'].get()
                    if val.startswith('V'): valve_bits[int(val[1:]) - 1] = 1
                    fields_order = ['pre_fill', 'tray_func', 'heater', 'tray_pos', 'fill_p', 'vacuum_p', 'dwell', 'min_t', 'max_t', 'bubble', 'calib', 'waste_p', 'heater_p']
                    rest_values = []
                    for field in fields_order:
                        if field in ['fill_p', 'vacuum_p', 'heater_p']: rest_values.append(int(row[field].get()))
                        elif field in ['dwell', 'min_t', 'max_t']: rest_values.append(int(round(row[field].get() * 10)))
                        else: rest_values.append(row[field].get())
                    rest_values.append(label_num)
                    line_parts = [str(idx + 1)] + [str(b) for b in valve_bits] + [str(v) for v in rest_values]
...
class ProtocolEditorEnhanced(ProtocolEditor):
    ...
    def _init_enhancements(self):
        ...
        for name_var in self.step_name_vars: name_var.trace_add("write", self._on_name_change)
        self._update_tab_titles()

    def _on_name_change(self, *args): self._update_tab_titles()

    def _update_tab_titles(self):
        for i in range(self.notebook.index("end")):
            name = self.step_name_vars[i].get().strip()
            self.notebook.tab(i, text=f"{i + 1}. {name or f'Staining Step {i + 1}'}")
    ...
    def _move_tab(self, direction):
        try:
            ...
            return  # This can happen if no tab is selected

        new_index = current_index + direction

        # Boundary check to prevent moving past the start or end
        if not (0 <= new_index < num_tabs):
            return

        # Reorder all backend data lists first
        ...
        # Reorder the tab visually in the notebook
        tab_widget = self.notebook.nametowidget(self.notebook.tabs()[current_index])
        self.notebook.forget(current_index)

        # THE CRITICAL FIX: Use "end" when inserting at the last position
        if new_index >= self.notebook.index("end"):
            self.notebook.insert("end", tab_widget, text="dummy")
        else:
