...
def get_user_input():
    print("""
    ЧТОБЫ WALLHACK РАБОТАЛ, ПОСТАВЬТЕ РАЗРЕШЕНИЕ В ИГРЕ:
    В ОКНЕ ЛИБО В ПОЛНОЭКРАННОМ РЕЖИМЕ В ОКНЕ
    """)
    print("FEAR NIGHT WALLHACK УСПЕШНО ЗАПУЩЕН")
    WINDOW_WIDTH = int(input("Введите ширину окна (например, 1920): "))
    WINDOW_HEIGHT = int(input("Введите высоту окна (например, 1080): "))
    esp_rendering = int(input("Включить WH? (0 - выключить, 1 - включить): "))
    esp_mode = int(input("Режим WH (0 - только враги, 1 - враги и тиммейты): "))
    line_rendering = int(input("Отрисовка линий (0 - не отрисовывать, 1 - отрисовка линий): "))
    hp_bar_rendering = int(input("Отрисовка полоски HP (0 - не отрисовывать, 1 - отрисовка полоски HP): "))
    head_hitbox_rendering = int(input("Отрисовка хитбокса головы (0 - не отрисовывать, 1 - отрисовка хитбокса головы): "))
    
    return WINDOW_WIDTH, WINDOW_HEIGHT, esp_rendering, esp_mode, line_rendering, hp_bar_rendering, head_hitbox_rendering

WINDOW_WIDTH, WINDOW_HEIGHT, esp_rendering, esp_mode, line_rendering, hp_bar_rendering, head_hitbox_rendering = get_user_input()
...
def main():
    if not glfw.init():
        print("Не удалось инициализировать OpenGL контекст")
        exit(1)
        
    glfw.window_hint(glfw.TRANSPARENT_FRAMEBUFFER, glfw.TRUE)
    window = glfw.create_window(WINDOW_WIDTH, WINDOW_HEIGHT, "FEAR NIGHT WH", None, None)
