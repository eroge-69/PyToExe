#This_program_can take data from detector via picoscope2204A OF 2000SERIES
#THIS_PROGRAM CAN PLOT VOLTAGE(IN mV,range +_50mV) VS. TIME(IN ms 150ms) channel A, DC COUPLING 
...
with ps2000.open_unit() as device:
    ...
    res = ps2000.ps2000_run_streaming_ns(
        device.handle,
        500,   #sample interval in ns(500ns ---2MS/s)
        2,     #time unit=ns(so 500*1=500ns) Note:0-femto,1-pico,2-nano,3-micro,4-milli
        100_000, #max samples
        False,    #true if we want to acquire only max sample,otherwise false
        1,        #downsampling   Note:1=no downsampling,2=avg 2 sample ratio 1,4=avg 4 sample
        50_000    #overview buffer size #used to store incoming data before we read it(when inbetween two location)
    )
    assert_pico2000_ok(res)

    start_time = time.time_ns()

    while time.time_ns() - start_time < 150_000_000:
