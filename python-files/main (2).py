...
def root_finding_app():
    ...
    while True:
        ...
        if choice == 'a':
            ...
            print(f"\nAccepted root ≈ {root:.10f} after {iters} iterations")
            print(f"Since |Δx| = {diff:.10f} <= ε = {tol}")
            ...
            root, iters, table, diff = secant_method(func_str, x0, x1, tol)
            if root is None:
                retry = input("\nMethod failed. Do you want to try again? (y/n): ").strip().lower()
                if retry != 'y':
                    print("\nThank you for using the Root-Finding App. Have a great day!")
            ...
            print("\nInvalid choice! Please choose a valid option (a/b).")
