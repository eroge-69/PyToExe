...
# Edit by Nightlancerx: now works for merged mods as well(searches through one-lvl subfolders), 
# supports direct execution with manual thickness input, defaults to 80 if none provided, displays new thickness
...
def main():
    ...
    if not thickness:
        print(f"\nEnter outline thickness:")
        thickness_ = input()
        if thickness_:
            thickness = int(thickness_)
        else: 
            thickness = 80
    ...
    for dir_ in dirs:
        texcoord_file = [x for x in os.listdir(dir_) if "Texcoord.buf" in x]
        ...
        ini_file = [x for x in os.listdir(dir_) if ".ini" in x]
        if len(ini_file) == 0:
        ...
        with open(os.path.join(dir_, texcoord_file), "rb+") as f:
            ...
            print(f"Writing results to new file ({thickness})")
            f.seek(0)
            f.write(data)
            f.truncate()
