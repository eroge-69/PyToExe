...
class Dota2AccountSwitcher:
    ...
    def _find_steam_path(self):
        try:
            ...
            print(f"[!] Couldnt find the path to Steam: {e}")
            return "C:\\Program Files (x86)\\Steam"
    
    def _find_dota2_path(self):
        steamapps = os.path.join(self.steam_path, "steamapps")
        common = os.path.join(steamapps, "common")
        dota_path = os.path.join(common, "Rust")
        return dota_path if os.path.exists(dota_path) else os.path.join(common, "Rust")
    ...
    def _kill_processes(self):
        print("[*] Close Steam and Rust...")
        processes = ["steam.exe", "steamservice.exe", "rust.exe"]
        killed = 0
    ...
    def _spoof_hwid(self):
        print("[*] New HWID generation...")
        new_hwid = random.getrandbits(128)
        print(f"[+] New Hardware ID: {new_hwid}")
    
    def _spoof_mac(self):
        print("[*] Generating a new MAC address...")
        new_mac = ":".join([f"{random.randint(0, 255):02x}" for _ in range(6)])
        print(f"[+] New Mac: {new_mac}")
    
    def _clear_steam_files(self):
        print("[*] Clearing Steam files...")
        paths_to_clear = [
    ...
    def _clear_userdata(self):
        print("\n[!] WARNING: ALL Steam user data will be deleted!")
        answer = input("Continue? (y/n): ").lower()
        if answer != 'y':
            print("[*] Skipped deleting userdata")
            return
            
        print("[*] Cleaning up the userdata folder...")
        userdata_path = os.path.join(self.steam_path, "userdata")
        
        if not os.path.exists(userdata_path):
            print("[-] Userdata folder not found")
            return
        ...
        try:
            for user_id in os.listdir(userdata_path):
                ...
                if os.path.isdir(user_folder):
                    ...
                    print(f"[+] User data removed {user_id}")
                    
            dota_userdata = glob.glob(os.path.join(userdata_path, "*", "252490"))
            for dota_path in dota_userdata:
                shutil.rmtree(dota_path)
                print("[+] Rust Data Deleted")
                
        except Exception as e:
            print(f"[-] Error during userdata cleanup: {e}")
    
    def _clear_dota2_files(self):
        print("[*] Clearing Rust files...")
        if not self.dota2_path:
            print("[-] Rust Folder Not Found")
            return
            
        paths_to_clear = [
            os.path.join(self.dota2_path, "game", "Rust", "cfg"),
            os.path.join(self.dota2_path, "game", "Rust", "save"),
            os.path.join(self.dota2_path, "game", "bin", "win64", "config"),
            os.path.join(os.getenv("LOCALAPPDATA"), "virtualstore", "program files (x86)", "steam", "steamapps", "common", "Rust*"),
            os.path.join(os.getenv("APPDATA"), "Microsoft", "Windows", "Recent", "*")
        ]
        
        cfg_files = ["config.cfg", "autoexec.cfg", "video.txt", "joyconfig.txt"]
        for cfg in cfg_files:
            paths_to_clear.extend(glob.glob(os.path.join(self.dota2_path, "game", "Rust", "cfg", cfg)))
    ...
    def _clear_registry(self):
        ...
        try:
            reg_paths = [
                ...
                r"Software\Valve\Steam\Valve\Rust"
            ]
