...
class ecueeprom:
    type =  "ECU"
    noFixCheckSum = False
    noCheckSumPages = [0x00,0x11,0x12,0x13]
        ...
        if self.size == 1024 or R32 == True:
            ...
            for page in range(0x20,0x40):
                for bite in self.data[page]:
                    if bite != 0xFF:
                        print "bite error 0x%0.2X = 0x%0.2X " % (page,bite)
                        R32=False
            if R32:
                ...
                print "1024byte file detected, but the last 512bytes are not all padded with 0xFF, this file may be a corupt dump, a cluster dump or something other than an eeprom dump, open it with a hex editor - if this is a valid eeprom please post the file and vehical info on the thread for this tool"
    ...
    def printStatus(self):
        ...
        print "- Flash Read/Write (successful): %d" % self.flashsuccessfulcount
        print "- Flash Read/Write (attempts): %d" % self.flashattemptcount
        print ""
...
#data = readbin("auq_van_95040.bin")
#data = readbin("Immo bin with SKC 05557.bin")
#data = readbin("ME7_95040_universal_immo_off.mod.bin")
...
def main():
    if WARN: print "95040 Eeeprom Tool - 0.3 - No warranty implied or given, manually verify all changes before saving eeprom to ECU, this tool could cause permenent damage to ECU and prevent vehicle running\n"
    parser = argparse.ArgumentParser(description='95040tool - View/Modify me7.5 eeprom files')
