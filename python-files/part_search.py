...
if sys.platform == "win32":
    try:
        ...
        pass  # Ignore errors if running in a non-standard environment
...
def search_part():
    ...
    for col, value in match.items():
        if col != search_column and value.strip():
            title_lbl = tk.Label(details_frame, text=col + ":", font=("Segoe UI", 12, "bold"),
                                 anchor="w", wraplength=160, justify="left")
            data_lbl = tk.Label(details_frame, text=value.strip(), font=("Segoe UI", 12),
            ...
            # Add bold black separators after certain fields
            if col.strip().upper() == "MATERIAL":
                sep = tk.Frame(details_frame, bg="black", height=3)
