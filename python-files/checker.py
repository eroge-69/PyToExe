...
def safe_input(prompt=""):
    """
    input() wrapper that won't crash when sys.stdin is not available (e.g. windowed/exe without console).
    Returns empty string if no input possible.
    """
    try:
        return input(prompt)
    except (EOFError, RuntimeError, OSError):
        # No stdin available
        print("[!] No interactive input available; proceeding with defaults where appropriate.")
        return ""
...
def cleaner_main(scan_results):
    ...
    typed = safe_input("Token: ").strip()
    if typed != token:
        print("[!] Confirmation token mismatch or no input. Aborting clean operation.")
        return
...
def ascii_title():
    print(r"PC CHECKER ANTI TUROK TANGINA NYO")
...
def menu():
    ...
    print("2 > CLEANER")
    print("q > QUIT")
    return safe_input("Choose option: ").strip().lower()

def main():
    choice = menu()
    # If no interactive input available (empty string), default to scanner.
    if choice == "":
        print("[*] No choice entered; defaulting to SCANNER.")
        choice = "1"
