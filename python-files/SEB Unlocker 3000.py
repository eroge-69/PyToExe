...
def decrypt_SEB(password, source_file, tmp_file):
	...
	# ---------------------------------------------------------------------
    	# 5. NEW FEATURE — save helper files for user inspection
	# ---------------------------------------------------------------------
    	try:
        	with open('str_data.txt', 'w', encoding='utf-8') as f_txt:
            		f_txt.write(str_data)
        	# bytes_data contains the same XML but encoded; decode again so we can
        	# store a readable text file (errors="replace" avoids crashes in the
        	# unlikely event of non‑UTF‑8 bytes).
        	with open('bytes_data.txt', 'w', encoding='utf-8') as f_bytes_txt:
            		f_bytes_txt.write(bytes_data.decode('utf-8', errors='replace'))
    	except Exception:
        	print('ERROR: Konnte str_data.txt oder bytes_data.txt nicht schreiben')
        	print(logging.exception(''))
        	# Not fatal for the main workflow – fall through.
