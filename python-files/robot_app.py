...
# -*- coding: utf-8 -*-
#
# Uygulama: Robot Yazılımı
# Açıklama: NVDA'nın wxPython tabanlı arayüz mimarisinden ilham alınarak geliştirilen,
#           erişilebilir bir robot kontrol ve sohbet uygulaması.
#           Özellikler: Sohbet, robota öğretme, makro oluşturma, ayarlar, ses çalma.
#
# İhtiyaçlar:
# - Python Sürümü: 3.8, 3.9, 3.10 veya 3.11 (3.12 ile wxPython uyumluluk sorunları olabilir)
# - Bağımlılıklar:
#   * wxPython: 4.2.1 veya üstü (`pip install wxPython==4.2.1`)
#   * pygame: Ses çalma için (`pip install pygame`)
# - İşletim Sistemi: Windows (NVDA ve wxPython ile tam uyumluluk), Linux/Mac (kısmi destek)
# - Opsiyonel: PyInstaller (`pip install pyinstaller`) ile bağımsız .exe oluşturmak için
# - Çalıştırma:
#   * Kaynak kodu için: `python robot_app.py` (hata ayıklama) veya `pythonw robot_app.pyw` (konsolsuz)
#   * Bağımsız .exe için: `pyinstaller --noconsole --onefile robot_app.pyw`
# - Not: Erişilebilirlik için NVDA ekran okuyucusuyla test edilmiştir.
#        conversations.json dosyası için yazma/okuma izinleri gereklidir.
#
...
class GuiHelper:
    ...
    def create_labeled_control(parent, label_text, control_class, **kwargs):
        try:
            ...
            logging.error(f"create_labeled_control hatası: {e}")
            raise
    ...
    def add_dialog_buttons(parent, on_save, on_cancel):
        try:
            ...
            logging.error(f"add_dialog_buttons hatası: {e}")
            raise
...
class MacroDialog(wx.Dialog):
    def __init__(self, parent, macro: Optional[MacroData] = None):
        try:
            ...
            logging.error(f"MacroDialog başlatma hatası: {e}")
            raise
    ...
    def on_save(self, event):
        try:
            ...
            logging.error(f"MacroDialog on_save hatası: {e}")
...
class TeachDialog(wx.Dialog):
    def __init__(self, parent, teach_data: Optional[TeachData] = None):
        try:
            ...
            logging.error(f"TeachDialog başlatma hatası: {e}")
            raise
    ...
    def add_question(self, event):
        try:
            ...
            logging.error(f"add_question hatası: {e}")
            wx.LogError(f"Soru ekleme hatası: {e}")
    ...
    def add_answer_field(self, text="", delay_text="Normal", sound=""):
        try:
            ...
            logging.error(f"add_answer_field hatası: {e}")
            wx.LogError(f"Cevap ekleme hatası: {e}")
    ...
    def select_sound(self, sound_input):
        try:
            ...
            logging.error(f"select_sound hatası: {e}")
            wx.LogError(f"Ses seçme hatası: {e}")
    ...
    def on_save(self, event):
        try:
            ...
            logging.error(f"TeachDialog on_save hatası: {e}")
            wx.LogError(f"Kaydetme hatası: {e}")
...
class SettingsDialog(wx.Dialog):
    def __init__(self, parent, settings: Settings):
        try:
            ...
            logging.error(f"SettingsDialog başlatma hatası: {e}")
            raise
    ...
    def add_system_sound(self, event):
        try:
            ...
            logging.error(f"add_system_sound hatası: {e}")
            wx.LogError(f"Ses ekleme hatası: {e}")
    ...
    def on_save(self, event):
        try:
            ...
            logging.error(f"SettingsDialog on_save hatası: {e}")
            wx.LogError(f"Kaydetme hatası: {e}")
...
class RobotApp(wx.App):
    def __init__(self):
        try:
            ...
            logging.error(f"RobotApp başlatma hatası: {e}")
            raise
    ...
    def OnInit(self):
        try:
            ...
            logging.error(f"OnInit hatası: {e}")
            wx.LogError(f"Uygulama başlatma hatası: {e}")
            return False
    ...
    def play_sound(self, sound_path):
        try:
            ...
            logging.error(f"play_sound hatası: {e}")
    ...
    def save_conversations(self):
        try:
            ...
            logging.error(f"save_conversations hatası: {e}")
            wx.LogError(f"Konuşmaları kaydetme hatası: {e}")
    ...
    def load_conversations(self):
        try:
            ...
            logging.error(f"load_conversations hatası: {e}")
            wx.LogError(f"Konuşmaları yükleme hatası: {e}")
...
class RobotFrame(wx.Frame):
    def __init__(self, parent, app: RobotApp):
        try:
            ...
            logging.error(f"RobotFrame başlatma hatası: {e}")
            raise
    ...
    def apply_theme(self):
        try:
            ...
            logging.error(f"apply_theme hatası: {e}")
            wx.LogError(f"Tema uygulama hatası: {e}")
    ...
    def get_answer_color(self, text):
        try:
            ...
            logging.error(f"get_answer_color hatası: {e}")
    ...
    def create_chat_page(self):
        try:
            ...
            logging.error(f"create_chat_page hatası: {e}")
            raise
    ...
    def create_teach_page(self):
        try:
            ...
            logging.error(f"create_teach_page hatası: {e}")
            raise
    ...
    def create_macro_page(self):
        try:
            ...
            logging.error(f"create_macro_page hatası: {e}")
            raise
    ...
    def create_settings_page(self):
        try:
            ...
            logging.error(f"create_settings_page hatası: {e}")
            raise
    ...
    def on_category_selected(self, event):
        try:
            ...
            logging.error(f"on_category_selected hatası: {e}")
            wx.LogError(f"Kategori seçme hatası: {e}")
    ...
    def show_teach_context_menu(self, event):
        try:
            ...
            logging.error(f"show_teach_context_menu hatası: {e}")
            wx.LogError(f"Sağ tıklama menüsü hatası: {e}")
    ...
    def add_teach_item(self, event):
        try:
            ...
            logging.error(f"add_teach_item hatası: {e}")
            wx.LogError(f"Öğretme ekleme hatası: {e}")
    ...
    def edit_teach_item(self, event):
        try:
            ...
            logging.error(f"edit_teach_item hatası: {e}")
            wx.LogError(f"Öğretme düzenleme hatası: {e}")
    ...
    def delete_teach_item(self, event):
        try:
            ...
            logging.error(f"delete_teach_item hatası: {e}")
            wx.LogError(f"Öğretme silme hatası: {e}")
    ...
    def update_teach_list(self):
        try:
            ...
            logging.error(f"update_teach_list hatası: {e}")
            wx.LogError(f"Öğrenilenler listesi güncelleme hatası: {e}")
    ...
    def add_macro_item(self, event):
        try:
            ...
            logging.error(f"add_macro_item hatası: {e}")
            wx.LogError(f"Makro ekleme hatası: {e}")
    ...
    def run_macro(self, event):
        try:
            ...
            logging.error(f"run_macro hatası: {e}")
            wx.LogError(f"Makro çalıştırma hatası: {e}")
    ...
    def update_macro_list(self):
        try:
            ...
            logging.error(f"update_macro_list hatası: {e}")
            wx.LogError(f"Makro listesi güncelleme hatası: {e}")
    ...
    def edit_settings(self, event):
        try:
            ...
            logging.error(f"edit_settings hatası: {e}")
            wx.LogError(f"Ayarlar düzenleme hatası: {e}")
    ...
    def send_chat_message(self, event):
        try:
            ...
            logging.error(f"send_chat_message hatası: {e}")
            wx.LogError(f"Mesaj gönderme hatası: {e}")
    ...
    def get_timestamp(self):
        try:
            ...
            logging.error(f"get_timestamp hatası: {e}")
            return ""
    ...
    def get_greeting(self):
        try:
            ...
            logging.error(f"get_greeting hatası: {e}")
            return "Merhaba"
    ...
    def add_chat_message(self, message, is_user=True):
        try:
            ...
            logging.error(f"add_chat_message hatası: {e}")
            wx.LogError(f"Mesaj ekleme hatası: {e}")
    ...
    def process_command(self, command, greeting):
        try:
            ...
            logging.error(f"process_command hatası: {e}")
            wx.LogError(f"Komut işleme hatası: {e}")
    ...
    def update_ui(self):
        try:
            ...
            logging.error(f"update_ui hatası: {e}")
            wx.LogError(f"UI güncelleme hatası: {e}")
...
if __name__ == "__main__":
    try:
        ...
        wx.MessageBox(f"Uygulama başlatılamadı: {e}", "Hata", wx.OK | wx.ICON_ERROR)