...
# === Auto-install required packages when running as .py ===
required_packages = [
    'stripe', 'cryptography', 'psutil', 'pandas', 'numpy', 'matplotlib',
    'seaborn', 'scipy', 'mplfinance', 'arch', 'pypfopt', 'ibapi'
]
...
# Only do installs when NOT running as a compiled .exe
...
DATABASE_FILE = os.path.join(os.environ['LOCALAPPDATA'], COMPANY_NAME, APP_NAME, 'users.db')
LICENSE_KEY_FILE = os.path.join(os.environ['LOCALAPPDATA'], COMPANY_NAME, APP_NAME, 'license.key')
ENCRYPTION_KEY = b'your_32_byte_encryption_key_here_replace_me'  # Must be 32 url-safe base64-encoded bytes
...
class LicensingError(TradingSystemError):
    """Exception raised for licensing issues"""
    pass

class PaymentError(TradingSystemError):
    """Exception raised for payment issues"""
    pass
...
class LicenseManager:
    """Handles licensing and payment functionality"""
    def __init__(self):
        self.fernet = Fernet(base64.urlsafe_b64encode(ENCRYPTION_KEY))
        self._setup_database()
        
    def _setup_database(self):
        """Setup the SQLite database for user accounts"""
    ...
    def create_user(self, email: str, password: str) -> str:
        ...
        try:
            cursor.execute('''
                INSERT INTO users (id, email, password_hash, salt, last_login)
                VALUES (?, ?, ?, ?, ?)
            ''', (user_id, email, password_hash, salt, datetime.datetime.now().isoformat()))
            self.conn.commit()
            return user_id
    ...
    def create_license_key(self, user_id: str, subscription_type: str) -> str:
        """Create a new license key for a user"""
        license_data = {
            'user_id': user_id,
            'subscription_type': subscription_type,
            'created': datetime.datetime.now().isoformat(),
            'expires': (datetime.datetime.now() + timedelta(days=365)).isoformat(),
            'signature': str(uuid.uuid4())
        }
        
        license_key = self.fernet.encrypt(json.dumps(license_data).encode())
        
        cursor = self.conn.cursor()
        cursor.execute('UPDATE users SET license_key = ?, subscription_type = ?, subscription_end = ? WHERE id = ?',
                      (license_key, subscription_type, license_data['expires'], user_id))
        self.conn.commit()
        
        return license_key
        
    def validate_license(self, license_key: str) -> dict:
        """Validate a license key and return its data"""
        try:
            decrypted = self.fernet.decrypt(license_key)
            license_data = json.loads(decrypted)
            
            # Check expiration
            expires = datetime.datetime.fromisoformat(license_data['expires'])
            if expires < datetime.datetime.now():
                raise LicensingError("License has expired")
                
            return license_data
        except:
            raise LicensingError("Invalid license key")
            
    def get_user_license(self, user_id: str) -> Optional[dict]:
        """Get a user's license information"""
        cursor = self.conn.cursor()
        cursor.execute('SELECT license_key FROM users WHERE id = ?', (user_id,))
        result = cursor.fetchone()
        
        if result and result[0]:
            return self.validate_license(result[0])
        return None
        
    def create_stripe_customer(self, user_id: str, email: str, payment_method: str) -> str:
    ...
    def create_subscription(self, user_id: str, subscription_type: str) -> str:
        ...
        # Define subscription prices (replace with your actual price IDs)
        price_ids = {
            'monthly': 'price_monthly_id',
            'yearly': 'price_yearly_id'
        }
        
        if subscription_type not in price_ids:
            raise PaymentError("Invalid subscription type")
            
        try:
            subscription = stripe.Subscription.create(
                customer=customer_id,
                items=[{'price': price_ids[subscription_type]}],
            ...
            if subscription.status == 'active':
                ...
                license_key = self.create_license_key(user_id, subscription_type)
                
                cursor.execute('''
                    UPDATE users 
                    SET subscription_type = ?, subscription_end = ?, license_key = ?
                    WHERE id = ?
                ''', (subscription_type, end_date.isoformat(), license_key, user_id))
                self.conn.commit()
                ...
                cursor.execute('''
                    ...
                    (id, user_id, amount, currency, payment_date, subscription_type, stripe_payment_id)
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                ''', (
                    payment_id,
                    user_id,
                ...
                raise PaymentError(f"Subscription not active: {subscription.status}")
        except stripe.error.StripeError as e:
            raise PaymentError(f"Stripe error: {str(e)}")
...
# [Rest of your original classes (PortfolioAnalyzer, IBApi, TradingEngine) remain exactly the same...]

class LoginWindow(tk.Toplevel):
    """Login window for the application"""
    def __init__(self, parent, license_manager: LicenseManager, on_success):
    ...
    def create_widgets(self):
        ...
        ttk.Label(self, text=f"Welcome to {APP_NAME}", font=('Helvetica', 14)).pack(pady=20)
        
        # Login frame
        login_frame = ttk.Frame(self)
        login_frame.pack(pady=10)
        
        ttk.Label(login_frame, text="Email:").grid(row=0, column=0, padx=5, pady=5, sticky=tk.E)
        ...
        ttk.Button(button_frame, text="Register", command=self.show_register).pack(side=tk.LEFT, padx=10)
        ttk.Button(button_frame, text="Exit", command=self.destroy).pack(side=tk.LEFT, padx=10)
    ...
    def login(self):
        ...
        try:
            user_id = self.license_manager.authenticate_user(email, password)
            
            # Check subscription status
            if not self.license_manager.check_subscription(user_id):
                messagebox.showwarning("Subscription Expired", 
                                     "Your subscription has expired. Please renew to continue using the application.")
                self.show_payment_window(user_id)
                return
    ...
    def show_register(self):
        ...
        RegisterWindow(self, self.license_manager)
        
    def show_payment_window(self, user_id: str):
        """Show payment window for subscription renewal"""
        PaymentWindow(self, self.license_manager, user_id)
...
class RegisterWindow(tk.Toplevel):
    ...
    def create_widgets(self):
        ...
        ttk.Label(self, text=f"Register for {APP_NAME}", font=('Helvetica', 14)).pack(pady=20)
        
        # Registration frame
        reg_frame = ttk.Frame(self)
        reg_frame.pack(pady=10)
        
        ttk.Label(reg_frame, text="Email:").grid(row=0, column=0, padx=5, pady=5, sticky=tk.E)
        self.email_entry = ttk.Entry(reg_frame, width=30)
        self.email_entry.grid(row=0, column=1, padx=5, pady=5)
        
        ttk.Label(reg_frame, text="Password:").grid(row=1, column=0, padx=5, pady=5, sticky=tk.E)
        self.password_entry = ttk.Entry(reg_frame, width=30, show="*")
        self.password_entry.grid(row=1, column=1, padx=5, pady=5)
        
        ttk.Label(reg_frame, text="Confirm Password:").grid(row=2, column=0, padx=5, pady=5, sticky=tk.E)
        self.confirm_entry = ttk.Entry(reg_frame, width=30, show="*")
        ...
        ttk.Button(button_frame, text="Register", command=self.register).pack(side=tk.LEFT, padx=10)
    ...
    def register(self):
        ...
        try:
            user_id = self.license_manager.create_user(email, password)
            messagebox.showinfo("Success", "Registration successful! Please subscribe to continue.")
            self.show_payment_window(user_id)
        except AuthenticationError as e:
            messagebox.showerror("Registration Failed", str(e))
            
    def show_payment_window(self, user_id: str):
        """Show payment window after successful registration"""
        PaymentWindow(self, self.license_manager, user_id)
        self.destroy()

class PaymentWindow(tk.Toplevel):
    """Payment window for subscriptions"""
    def __init__(self, parent, license_manager: LicenseManager, user_id: str):
        super().__init__(parent)
        self.title(f"{APP_NAME} - Subscribe")
    ...
    def create_widgets(self):
        ...
        ttk.Label(self, text=f"Subscribe to {APP_NAME}", font=('Helvetica', 14)).pack(pady=20)
        
        # Subscription options
        sub_frame = ttk.LabelFrame(self, text="Subscription Options", padding=10)
        sub_frame.pack(pady=10, padx=20, fill=tk.X)
        
        self.sub_var = tk.StringVar(value="monthly")
        
        ttk.Radiobutton(sub_frame, text="Monthly Subscription - $29.99/month", 
                       variable=self.sub_var, value="monthly").pack(anchor=tk.W, pady=5)
        ttk.Radiobutton(sub_frame, text="Annual Subscription - $299.99/year (Save 20%)", 
                       variable=self.sub_var, value="yearly").pack(anchor=tk.W, pady=5)
        
        # Payment method
        pay_frame = ttk.LabelFrame(self, text="Payment Method", padding=10)
        pay_frame.pack(pady=10, padx=20, fill=tk.X)
        
        ttk.Label(pay_frame, text="Card Number:").pack(anchor=tk.W)
        self.card_entry = ttk.Entry(pay_frame)
        self.card_entry.pack(fill=tk.X, pady=5)
        
        ttk.Label(pay_frame, text="Expiration (MM/YY):").pack(anchor=tk.W)
        self.expiry_entry = ttk.Entry(pay_frame)
        self.expiry_entry.pack(fill=tk.X, pady=5)
        
        ttk.Label(pay_frame, text="CVC:").pack(anchor=tk.W)
        self.cvc_entry = ttk.Entry(pay_frame)
        self.cvc_entry.pack(fill=tk.X, pady=5)
        
        # Buttons
        button_frame = ttk.Frame(self)
        button_frame.pack(pady=20)
        
        ttk.Button(button_frame, text="Subscribe", command=self.process_payment).pack(side=tk.LEFT, padx=10)
        ttk.Button(button_frame, text="Cancel", command=self.destroy).pack(side=tk.LEFT, padx=10)
    ...
    def process_payment(self):
        ...
        sub_type = self.sub_var.get()
        
        if not card or not expiry or not cvc:
        ...
        try:
            ...
            # Get user email
            cursor = self.license_manager.conn.cursor()
            cursor.execute('SELECT email FROM users WHERE id = ?', (self.user_id,))
            email = cursor.fetchone()[0]
            
            # Create customer and attach payment method
            customer_id = self.license_manager.create_stripe_customer(self.user_id, email, payment_method.id)
            
            # Create subscription
            sub_id = self.license_manager.create_subscription(self.user_id, sub_type)
            
            messagebox.showinfo("Success", "Subscription activated successfully!")
            self.destroy()
        except PaymentError as e:
            messagebox.showerror("Payment Failed", str(e))
        except stripe.error.StripeError as e:
            messagebox.showerror("Payment Failed", f"Payment processing error: {str(e)}")
        except Exception as e:
            messagebox.showerror("Error", f"An unexpected error occurred: {str(e)}")

class TradingApp(tk.Tk):
    """Main trading application with licensing integration"""
    def __init__(self, user_id: str):
        ...
        self.after(3600000, self.check_license_status)  # Check every hour
        
    def check_license_status(self):
        """Check if the user's license is still valid"""
        try:
            if not self.license_manager.check_subscription(self.user_id):
                messagebox.showwarning("Subscription Expired", 
                                     "Your subscription has expired. Please renew to continue using the application.")
                PaymentWindow(self, self.license_manager, self.user_id)
        except Exception as e:
            logger.error(f"Error checking license status: {str(e)}")
            
        # Schedule next check
        self.after(3600000, self.check_license_status)

    # [All other methods from your original TradingApp class remain exactly the same...]
...
def main():
        ...
        try:
            with open(LICENSE_KEY_FILE, 'r') as f:
                ...
                if user_id and license_key:
                    ...
                    license_valid = license_manager.check_subscription(user_id)
        except:
            pass
    ...
    if not license_valid:
        ...
        def on_login_success(uid):
            nonlocal login_success, user_id
            login_success = True
            user_id = uid
            # Save license info for future launches
            with open(LICENSE_KEY_FILE, 'w') as f:
                json.dump({'user_id': uid}, f)
        
        login_window = LoginWindow(root, license_manager, on_login_success)
