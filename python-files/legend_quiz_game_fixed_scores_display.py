...
class LegendQuizGame:
    ...
    def update_score_label(self):
        # Recalculate total and update the main menu label if it exists
        score_total = 0
        try:
            for country in self.difficulty_results.values():
                for diff in country.values():
                    score_total += int(diff.get("score", 0))
        except Exception:
            pass
        if hasattr(self, 'score_label') and self.score_label:
            try:
                self.score_label.config(text=f"Score: {score_total}")
            except Exception:
                pass
    ...
    def show_main_menu(self):
        ...
        # Score label (shown at top in main menu)
        # Score label (shown at top in main menu)
        score_total = 0
        try:
            for country in self.difficulty_results.values():
