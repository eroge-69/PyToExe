...
def main(argv):
    ...
    if msg.command == ADD_COMMAND:
        ...
        #keys = [alert["rule"]["id"]]
        keys = [alert["data"]["win"]["eventdata"]["targetUserName"]]
        """ End Custom Key """
        ...
        dc_list=[]
        cn_list=[]
        command=f"""dsquery user -name {username}"""
        result = subprocess.run(command, shell=True, capture_output=True,text=True)
        data=result.stdout
        data_list=data.split(',')
        for item in data_list:
            if item.startswith("\"CN="):
                cn_list.append(item.split('=')[1])
            elif item.startswith("OU="):
                ou_list.append(item.split('=')[1])
            elif item.startswith("DC="):
                item1=item.replace("\n","")
                dc_list.append(item1.split('=')[1])
        command1="dsmod user \""
