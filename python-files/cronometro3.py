...
# Seleção de serviço
tk.Label(janela, text="Selecione o serviço:").pack(pady=5)
combo_servicos = tk.StringVar()
menu = tk.OptionMenu(janela, combo_servicos, *servicos.keys(), command=selecionar_servico)
menu.pack()

label_valor_info = tk.Label(janela, text="Nenhum serviço selecionado")
label_valor_info.pack(pady=5)

# Labels
label_tempo = tk.Label(janela, text="0.00 s", font=("Arial", 20))
label_tempo.pack(pady=5)

label_valor = tk.Label(janela, text="R$ 0.00", font=("Arial", 16))
...
btn_play = tk.Button(frame_botoes, text="▶ Play", command=iniciar)
btn_play.grid(row=0, column=0, padx=5)

btn_pause = tk.Button(frame_botoes, text="⏸ Pausar", command=pausar)
btn_pause.grid(row=0, column=1, padx=5)

btn_reset = tk.Button(frame_botoes, text="⏹ Zerar", command=zerar)
btn_reset.grid(row=0, column=2, padx=5)

btn_salvar = tk.Button(janela, text="💾 Salvar registro", command=salvar)
