...
parser.add_argument('-s','--indexshape',type=int, help='Specify the shape to inject into', required=False)
...
def main():
    ...
    readCSV(cvsIn)
    readBFRES(f, args.indexfmdl, args.indexshape)
...
def calculation_tans(x, y, z, u, v, w): #World space positions and tex coords
    D = y-x
    E = z-x
...
def readBFRES(f, Index, ShapeIndex): #This index variable is for the FMDL choosen.
    ...
    if (SwitchCheck == 0x20202020): #Check magic padding from fres....
        ...
        for mdl in range(1, FMDLTotal + 1):
            ...
            for m in range(len(FSHPArr)):
                ...
                if ShapeIndex != None:
                    if ShapeIndex > len(FSHPArr):
                        print('Error. Shape index you typed in is too high than shape count (' + str(len(FSHPArr)) + ')')
                        break
                    elif ShapeIndex <= 0:
                        print('Error. Shape index you typed in is too low. It starts at 1 then goes up')
                        break
                    elif m != ShapeIndex - 1:
                        continue
              ...
              #  for item in BuffArr:
                ...
                SubMeshOff = readu64(f)
                unk1 = readu64(f)
                unk2 = readu64(f)
                IndexBufferSizeOff = readu64(f)
               ...
               # print("Datastart = " + str(Datastart))
                ...
                if (FSHPArr[m].lodMdlCount > 1):
                       for something in range(FSHPArr[m].lodMdlCount - 1):
                                                ...
                                                for iii in range(2):
                                                        if(len(bwgt)<(iii+1)):
                                                                ...
                                                                if(valWeght>maxWeght):
                                                                        ...
                                                                        maxWeght -= valWeght
                                                                writeByte(f,valWeght)
                                if(AttrArr[attr].attName == "_i1"):
                                        if(AttrArr[attr].vertType == 779):
                                                for iii in range(4):
                                                        if(len(binx)<(4+iii+1)):
                                                                writeByte(f,0)
                                                        else:
                                                                writeByte(f,binx[iii + 4])
                                        elif(AttrArr[attr].vertType == 777):
                                                for iii in range(2):
                                                        if(len(binx)<(4+iii+1)):
                                                                writeByte(f,0)
                        ...
                        for attr in range(len(AttrArr)):
                                ...
                                if(AttrArr[attr].attName == "_c0"):
                                        ...
                                        if(AttrArr[attr].vertType == 2063):
                                                for clr in color0:
                                                       clr = float(clr)
                                                       f.write(struct.pack(">H", halffloat(clr/255)))
                                if(AttrArr[attr].attName == "_u0"):
                                ...
                                if(AttrArr[attr].attName == "_i1"):
                                        if(AttrArr[attr].vertType == 266):
                                                ...
                                                writeByte(f,binx[0])
                                        else:print("Unk %i for _i1" % AttrArr[attr].vertType)
                                if(AttrArr[attr].attName == "_w0"):
                                        if(AttrArr[attr].vertType == 10):
                                                maxWeght = 255
                                                for iii in range(4):
