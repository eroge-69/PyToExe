# notapro.py (versão com alteração de fonte por trecho)
...
# --- NOVA JANELA DE DIÁLOGO PARA FONTES ---
class FontDialog(tk.Toplevel):
    def __init__(self, parent):
        super().__init__(parent)
        self.transient(parent)
        self.title("Escolher Fonte")
        self.geometry("400x400")
        self.result = None

        # Lista de fontes comuns para simplificar a escolha
        ...
        # --- Widgets ---
        # Frame para a lista de fontes
        font_frame = tk.Frame(self)
        font_frame.pack(pady=10, padx=10, fill=tk.X)
        ...
        # Frame para estilo e tamanho
        style_frame = tk.Frame(self)
        style_frame.pack(pady=5, padx=10, fill=tk.X)
        ...
        # Frame de preview
        preview_frame = tk.LabelFrame(self, text="Amostra", padx=10, pady=10)
        preview_frame.pack(pady=10, padx=10, fill=tk.BOTH, expand=True)
        self.preview_label = tk.Label(preview_frame, text="AaBbYyZz", font=("Arial", 12))
    ...
    def _update_preview(self, event=None):
        ...
        weight = "bold" if self.is_bold.get() else "normal"
        slant = "italic" if self.is_italic.get() else "roman"
    ...
    def _on_ok(self):
        ...
        self.result = (family, size, weight, slant)
        self.destroy()
...
class NotaPro(tk.Tk):
    def __init__(self):
        ...
        self._create_widgets(); self._create_menu(); self._bind_shortcuts()
    ...
    def _create_menu(self):
        ...
        # --- NOVO ITEM DE MENU ---
        format_menu.add_command(label="Alterar fonte do trecho...", command=self._change_selection_font)
    ...
    def _bind_shortcuts(self):
        ...
        self.bind("<Control-u>", lambda e: self._apply_colored_underline()); self.text_widget.bind("<MouseWheel>", self._zoom_on_scroll)
    ...
    def _save_npro_file(self, filepath):
        ...
        for tag_name in self.text_widget.tag_names():
            if tag_name.startswith("style_"):
                ...
                font_config = self.text_widget.tag_cget(tag_name, "font")
                fg = self.text_widget.tag_cget(tag_name, "foreground"); bg = self.text_widget.tag_cget(tag_name, "background"); ul = self.text_widget.tag_cget(tag_name, "underline")
                if font_config:
                    f = font.Font(font=font_config)
                    config["font"] = [f.actual("family"), f.actual("size"), f.actual("weight"), f.actual("slant")]
    ...
    # --- NOVA FUNÇÃO PARA ALTERAR FONTE DA SELEÇÃO ---
    def _change_selection_font(self):
        if not self.text_widget.tag_ranges(tk.SEL):
            messagebox.showinfo("Nenhuma Seleção", "Selecione um trecho de texto para formatar a fonte.")
            return
    ...
    def _highlight_selection(self):
        ...
        tag_name = f"style_{self.style_counter}"; self.style_counter += 1
        self.text_widget.tag_add(tag_name, "sel.first", "sel.last"); self.text_widget.tag_config(tag_name, background=blended_color)
        self._on_text_change(None)
    ...
    def _apply_colored_underline(self):
        ...
        tag_name = f"style_{self.style_counter}"; self.style_counter += 1
        self.text_widget.tag_add(tag_name, "sel.first", "sel.last")
        self.text_widget.tag_config(tag_name, foreground=chosen_color[1], underline=True)
        self._on_text_change(None)
