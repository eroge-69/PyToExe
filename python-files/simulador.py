# animalitos.py - Sistema de analisis para Loteria Animalitos
import os
import pickle
...
class AnalizadorAnimalitos:
    def __init__(self):
        self.animales = [
            ...
            "Leon", "Lobo", "Mariposa", "Perro", "Pescado",
            "Pez", "Rana", "Raton", "Serpiente", "Tigre"
        ]
    ...
    def mostrar_estadisticas(self):
        ...
        print(f"\n{'='*60}")
        print("SISTEMA DE ANALISIS DE LOTERIA ANIMALITOS")
        print(f"{'='*60}")
        print(f"Total de resultados: {stats['total_resultados']}")
        print(f"Ultima actualizacion: {stats['ultima_actualizacion']}")
        print(f"Animal mas probable: {stats['animal_mas_probable']} ({stats['probabilidad_maxima']:.2%})")
        print(f"\nTOP 5 ANIMALES:")
...
def main():
    print("Iniciando sistema de analisis de Loteria Animalitos...")
    ...
    while True:
        try:
            ...
            if opcion == "1":
                ...
                confirmacion = input("¿Esta seguro de reiniciar todos los datos? (s/n): ").strip().lower()
                if confirmacion == 's':
