...
def main():
    ...
    while True:
        intrare = input("Introdu număr de Plain: ")
        if intrare.lower() in ["exit", "iesire", "stop"]:
            break
        if not intrare.isdigit():
            print("⚠️ Te rog să introduci un număr valid.")
            continue
        ...
        print("Rezultatul a fost salvat în 'istoric.txt'.\n")

    print("Program încheiat. Mulțumim!")
