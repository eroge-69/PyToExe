...
def show_notification(room, sender, message_text):
    ...
    # We escape quotes and curly braces for the f-string.
    ps_script = f"""
    [Windows.UI.Notifications.ToastNotificationManager, Windows.UI.Notifications, ContentType = WindowsRuntime] | Out-Null
    $template = [Windows.UI.Notifications.ToastNotificationManager]::GetTemplateContent([Windows.UI.Notifications.ToastTemplateType]::ToastText02)
    $xml = $template.GetXml()
    $xml.GetElementsByTagName('text')[0].AppendChild($template.CreateTextNode("{title}")) | Out-Null
    $xml.GetElementsByTagName('text')[1].AppendChild($template.CreateTextNode("{msg_body}")) | Out-Null
    $toast = [Windows.UI.Notifications.ToastNotification]::new($xml)
    [Windows.UI.Notifications.ToastNotificationManager]::CreateToastNotifier('ChatterBoxLocal').Show($toast)
    """
    # Use subprocess to run the PowerShell command.
    # This is run in a separate thread to avoid blocking the GUI.
...
def send_presence_heartbeat():
    ...
    sock.sendto(json.dumps(payload).encode('utf-8'), ('<broadcast>', 6767))
    root.after(10000, send_presence_heartbeat)
...
def update_online_users_display():
    ...
    root.after(1000, update_online_users_display)
...
def update_typing_status_display():
    ...
    # Reschedule the check
    root.after(1000, update_typing_status_display)
