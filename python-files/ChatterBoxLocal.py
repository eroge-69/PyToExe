...
def show_notification(room, sender, message_text):
    ...
    # Using a library is more robust for notifications.
    # For now, let's ensure PowerShell command is correctly escaped and constructed.
    # The title and message need to be escaped for PowerShell.
    escaped_title = title.replace("'", "''")
    escaped_msg = msg_body.replace("'", "''")

    ps_command = f"""[Windows.UI.Notifications.ToastNotificationManager, Windows.UI.Notifications, ContentType = WindowsRuntime] | Out-Null; $template = [Windows.UI.Notifications.ToastNotificationManager]::GetTemplateContent([Windows.UI.Notifications.ToastTemplateType]::ToastText02); $xml = $template.GetXml(); $xml.GetElementsByTagName('text')[0].AppendChild($template.CreateTextNode('{escaped_title}')) | Out-Null; $xml.GetElementsByTagName('text')[1].AppendChild($template.CreateTextNode('{escaped_msg}')) | Out-Null; $toast = [Windows.UI.Notifications.ToastNotification]::new($xml); [Windows.UI.Notifications.ToastNotificationManager]::CreateToastNotifier('ChatterBoxLocal').Show($toast)"""
    
    threading.Thread(target=lambda: subprocess.run(["powershell", "-Command", ps_command], capture_output=True, text=True), daemon=True).start()
