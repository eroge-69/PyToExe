...
def DotPlace():
    ...
    for x in range(int(xSize)):
        for y in range(int(ySize)):
            ...
            # Smooth adjustable slow mode
            if slowMode.get():
                delay_ms = slowDelayScale.get()  # get slider value
                time.sleep(delay_ms / 1000)      # convert ms to seconds
            else:
                if random.randint(1, 5) == 1:  # occasional pause when not in slow mode
                    time.sleep(0.01)
...
tk.Radiobutton(root, text="URL", padx=20, variable=imageMode,
               command=ChangeImageMode, value=1).pack()
tk.Radiobutton(root, text="Local File", padx=20, command=ChangeImageMode,
               variable=imageMode, value=2).pack()
tk.Radiobutton(root, text="Dot Placement", padx=20, variable=drawMode,
               command=ChangeImageMode, value=0).pack()
tk.Radiobutton(root, text="Line Placement", padx=20, variable=drawMode,
               command=ChangeImageMode, value=1).pack()
...
# New: adjustable delay slider
slowDelayScale = tk.Scale(root, from_=10, to=500, orient="horizontal", label="Slow Mode Delay (ms)")
slowDelayScale.set(50)
slowDelayScale.pack()
