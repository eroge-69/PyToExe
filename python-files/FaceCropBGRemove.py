...
os.environ["NO_CUDA"] = "1"  # Additional flag to ensure no CUDA
...
def remove_background(cropped_image):
    """Remove background using RMBG model with transformers pipeline"""
    # Convert to PIL Image
    pil_image = Image.fromarray(cv2.cvtColor(cropped_image, cv2.COLOR_BGR2RGB))
    
    # Create pipeline with device set to CPU
    pipe = pipeline(
        "image-segmentation", 
    ...
    white_bg.paste(pil_image, mask=mask)
