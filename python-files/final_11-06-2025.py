...
class MKVSubtitleEditor:
    def __init__(self, root):
        ...
        self.mkvmerge = self._find_tool("mkvmerge.exe")  # Corrigido para _find_tool
        self.mkvpropedit = self._find_tool("mkvpropedit.exe")  # Corrigido para _find_tool
    ...
    def _find_tool(self, tool_name):  # Método renomeado para _find_tool
    ...
    def process_subtitles(self):
        """Process subtitles - set Portuguese subtitle as default"""
        selected_files = [self.file_listbox.get(i) for i in self.file_listbox.curselection()]
        if not selected_files:
            messagebox.showwarning("Aviso", "Selecione pelo menos um arquivo!")
            return
        ...
        for i, filepath in enumerate(selected_files, 1):
            try:
                ...
                # Find Portuguese subtitle (prioritize PT-PT, then PT-BR, then any PT)
                pt_subtitle = None
                pt_variants = ['pt-pt', 'pt', 'por', 'pt-br']  # Order of priority
                ...
                for variant in pt_variants:
                    for track in subtitles:
                        ...
                        if variant in lang or variant in track_name:
                            pt_subtitle = track
                            break
                    if pt_subtitle:
                        break
                ...
                if not pt_subtitle:
                    self.update_progress(i/total*100, f"Nenhuma legenda PT encontrada em {os.path.basename(filepath)}")
                    errors += 1
                    continue
                ...
                for track in subtitles:
                    subprocess.run(
                        [self.mkvpropedit, filepath, '--edit', f'track:{track["id"]}', 
                         '--set', 'flag-default=0', '--set', 'flag-forced=0'],
                        check=True,
                        stderr=subprocess.DEVNULL
                    )
                
                # Set Portuguese subtitle as default
                subprocess.run(
                    [self.mkvpropedit, filepath, '--edit', f'track:{pt_subtitle["id"]}', 
                ...
                errors += 1
                error_msg = f"Erro em {os.path.basename(filepath)}: {str(e.stderr).strip()}"
        ...
        if success > 0:
            self.update_progress(100, f"Legenda PT definida como padrão em {success} arquivo(s)!")
            messagebox.showinfo("Sucesso", 
                             f"Legenda PT definida como padrão em {success} arquivo(s)!")
