...
class ConfigWindow(QtWidgets.QWidget):
    ...
    def create_esp_container(self):
        ...
        self.hp_bar_rendering_cb = QtWidgets.QCheckBox("Показывать полоски HP")
        ...
        self.head_hitbox_rendering_cb = QtWidgets.QCheckBox("Показывать хитбокс головы")
        ...
        self.bons_cb = QtWidgets.QCheckBox("Показывать скелет")
    ...
    def create_trigger_container(self):
        ...
        trigger_layout.addWidget(QtWidgets.QLabel("Клавиша триггера:"))
    ...
    def create_aim_container(self):
        ...
        aim_label = QtWidgets.QLabel("Настройки Аима")
        ...
        aim_layout.addWidget(QtWidgets.QLabel("Радиус аима:"))
        ...
        aim_layout.addWidget(QtWidgets.QLabel("Клавиша аима:"))
        ...
        aim_layout.addWidget(QtWidgets.QLabel("Режим аима:"))
        ...
        aim_layout.addWidget(QtWidgets.QLabel("Режим определения дистанции:"))
...
class ESPWindow(QtWidgets.QWidget):
    ...
    def update_scene(self):
        ...
        try:
            ...
            fps_text = self.scene.addText(f"PyIt | ФПС: {self.fps}", QtGui.QFont('DejaVu Sans', 12, QtGui.QFont.Bold))
            fps_text.setPos(5, 5)
...
if __name__ == "__main__":
    ...
    try:
        ...
        while True:
            ...
            # Если хотя бы один из процессов завершился, считаем, что чит нужно перезапустить
            procs_alive = all([
                esp_proc is not None and esp_proc.is_alive(),
                trigger_proc is not None and trigger_proc.is_alive(),
                aim_proc is not None and aim_proc.is_alive()
            ]) if cs2_was_running else False
            if cs2_running and (not cs2_was_running or not procs_alive):
                if cs2_was_running and not procs_alive:
                    # Завершаем старые процессы, если они были
                    for proc in [esp_proc, trigger_proc, aim_proc]:
                    ...
                    trigger_proc = None
                    aim_proc = None
                print("CS2 обнаружен! Запуск чита...")
                esp_proc = multiprocessing.Process(target=esp_main)
                trigger_proc = multiprocessing.Process(target=triggerbot)
                aim_proc = multiprocessing.Process(target=aim)
                esp_proc.start()
                trigger_proc.start()
                aim_proc.start()
                cs2_was_running = True
            elif (not cs2_running and cs2_was_running):
                print("CS2 закрыт. Остановка чита...")
                for proc in [esp_proc, trigger_proc, aim_proc]:
                    if proc is not None and proc.is_alive():
                        proc.terminate()
                        proc.join()
        ...
        for proc in [process1, esp_proc, trigger_proc, aim_proc]:
            if proc is not None and proc.is_alive():
                proc.terminate()
                proc.join()