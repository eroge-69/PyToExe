...
FIO_MARKERS = ["111", "222", "333", "444", "555"]
POSITION_MARKERS = ["11", "22", "33", "44", "55"]

# Данные по сотрудникам (ФИО: должность)
EMPLOYEES = {
    "Перекин Д.А.": "Инженер по охране труда",
    "Горбачев О.Н.": "Начальник участка",
    "Ласточкина Е.Н.": "Главный инженер"
}
...
class PermitApp:
    ...
    def save_brigade(self):
        try:
            with open("brigade.json", "w", encoding="utf-8") as f:
    ...
    def load_brigade(self):
        ...
        if os.path.exists("brigade.json"):
            try:
                with open("brigade.json", "r", encoding="utf-8") as f:
