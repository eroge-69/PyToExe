...
def load_today_draw_numbers(path=None):
    global today_draws, today_file_path
    today_draws.clear()
    if path:
        today_file_path = path
    if not today_file_path:
        messagebox.showwarning("Missing File", "Please upload Today.txt first.")
        return
    try:
        ...
        messagebox.showerror("Error", f"Failed to load Today.txt:\n{e}")

def load_dealer_names(path=None):
    global dealer_names, name_file_path
    dealer_names.clear()
    if path:
        name_file_path = path
    if not name_file_path:
        messagebox.showwarning("Missing File", "Please upload name.txt first.")
        return
    try:
        ...
        messagebox.showerror("Error", f"Failed to load name.txt:\n{e}")
...
def upload_file(file_type):
    ...
    if file_path:
        if file_type == "today":
            load_today_draw_numbers(file_path)
        elif file_type == "name":
            load_dealer_names(file_path)
        apply_search_filter()
...
today_btn = tk.Button(top_frame, text="Upload Today.txt", command=lambda: upload_file("today"), font=("Calibri", 12))
today_btn.pack(side=tk.LEFT, padx=10)

name_btn = tk.Button(top_frame, text="Upload name.txt", command=lambda: upload_file("name"), font=("Calibri", 12))
name_btn.pack(side=tk.LEFT, padx=10)
