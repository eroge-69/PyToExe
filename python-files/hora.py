...
ACTIVATION_FILE = "act.dat"
ACTIVATION_CONSTANT = 1357 # Constante para generar el código
...
class BuscadorApp:
    ...
    def sort_by(self, col, descending):
        ...
        try:
            # Intenta ordenar numéricamente si es posible
            data.sort(key=lambda t: int(str(t[0]).strip()), reverse=descending)
        except (ValueError, TypeError):
            # Si falla, ordena alfabéticamente
...
def check_and_run_app():
    """
    Verifica la activación. Si no está activado, solicita el código.
    Si la activación es exitosa (o ya estaba activado), ejecuta la app principal.
    """
    # Si el archivo de activación ya existe, simplemente retorna True.
    if os.path.exists(ACTIVATION_FILE):
        return True

    # --- Lógica de activación para la primera vez ---
    # Es necesario crear una ventana raíz temporal para que los diálogos funcionen.
    # La ocultamos inmediatamente para que el usuario no la vea.
    temp_root = tk.Tk()
    temp_root.withdraw()

    # Genera el código correcto basado en la hora actual
    hora_actual = datetime.datetime.now().hour
    codigo_correcto = str(hora_actual * ACTIVATION_CONSTANT)

    # Pide el código al usuario
    codigo_usuario = simpledialog.askstring("Activación Requerida", 
                                            "Por favor, introduce el código de activación para continuar:",
                                            parent=temp_root)

    # Verifica si el código es correcto
    if codigo_usuario == codigo_correcto:
        try:
            # Si es correcto, crea el archivo de activación
            with open(ACTIVATION_FILE, 'w') as f:
                f.write(datetime.datetime.now().isoformat())
            messagebox.showinfo("Activación Exitosa", "El producto ha sido activado correctamente.", parent=temp_root)
            temp_root.destroy()
            return True # Señal para proceder
        except IOError as e:
            messagebox.showerror("Error Crítico", f"No se pudo crear el archivo de activación: {e}", parent=temp_root)
            temp_root.destroy()
            return False # Señal para no continuar
    else:
        # Si es incorrecto o se cancela, muestra un error y sale
        messagebox.showerror("Activación Fallida", "El código de activación es incorrecto. El programa se cerrará.", parent=temp_root)
        temp_root.destroy()
        return False # Señal para no continuar


if __name__ == "__main__":
    # Primero, se ejecuta la verificación de activación.
    # La aplicación principal solo se lanzará si check_and_run_app() devuelve True.
    if check_and_run_app():
        root = tk.Tk()
