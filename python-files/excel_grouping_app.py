        ...
        try:
            ...
            for row in range(2, ws_source.max_row + 1):
                ...
                if date_cell:
                    if isinstance(date_cell, datetime):
                    ...
                    elif isinstance(date_cell, (int, float)):  # Excel date number
                        try:
                        ...
                        try:
                            date_value = datetime.strptime(date_cell, "%Y-%m-%d %H:%M:%S")
                        except:
                            try:
                                date_value = datetime.strptime(date_cell, "%d/%m/%Y")
                            except:
                                continue
                    else:
                        continue
            ...
            for tag in tags:
                ...
                for row in range(2, ws_source.max_row + 1):
                    ...
                    if tag in [t.strip() for t in tag_cell_value.split(",")]:
                        ...
                        # Amount
                        amount = ws_source.cell(row=row, column=16).value
                        if amount is None:
                            amount = 0
                        try:
                            amount = float(amount)
            ...
            for row in range(4, current_row):
                for col in range(1, 6):
                    ws_pcv.cell(row=row, column=col).border = thin_border
