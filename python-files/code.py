...
class PyNotePlus:
    def __init__(self, root):
        ...
        self.default_bg = "#FFFFFF" # Branco
        self.default_fg = "#000000" # Preto

        # Área de texto principal com rolagem
        self.text_area = scrolledtext.ScrolledText(
            ...
            bg=self.default_bg,
            fg=self.default_fg,
            insertbackground=self.default_fg # Cor do cursor
        )
        ...
        self.root.bind('<Control-S>', self.save_as_file) # Ctrl+Shift+S
    ...
    def new_file(self, event=None):
        ...
        self.update_title()
        # Limpa todas as tags de formatação antigas
    ...
    def open_file(self, event=None):
        ...
        try:
            self.new_file() # Limpa o editor antes de abrir
            self.current_file_path = file_path
            
            # Lógica para carregar o formato .pynote
            if file_path.endswith('.pynote'):
                with open(file_path, 'r', encoding='utf-8') as file:
                    data = json.load(file)
                    
                    # Carregar metadados (cores globais)
                    ...
                    # Carregar e aplicar tags de cor
    ...
    def save_file(self, event=None):
        if self.current_file_path and self.current_file_path.endswith('.pynote'):
            self._save_logic(self.current_file_path)
        elif self.current_file_path and self.current_file_path.endswith('.txt'):
             self._save_logic(self.current_file_path)
        else:
            self.save_as_file()
    ...
    def _save_logic(self, file_path):
        try:
            ...
            # Salvar no formato .pynote
            else:
                content = self.text_area.get('1.0', 'end-1c') # Exclui a nova linha final automática
                
                tags_data = []
                # Coleta todas as tags que definem cores
                ...
                for tag_name in tag_names:
                    ...
                    if tag_ranges:
                        # Converte os TclObjects em pares de strings
    ...
    def change_selected_text_color(self):
        try:
            ...
            if self.text_area.tag_ranges("sel"):
                ...
                if color_code and color_code[1]:
                    # Cria uma tag única para esta cor e seleção
                    # Usar o tempo em microssegundos garante um nome de tag único
                    tag_name = f"color_{int(time.time() * 1000000)}"
                    self.text_area.tag_configure(tag_name, foreground=color_code[1])
                    self.text_area.tag_add(tag_name, "sel.first", "sel.last")
        except tk.TclError:
            # Isso acontece se não houver seleção. Podemos ignorar silenciosamente.
            pass
