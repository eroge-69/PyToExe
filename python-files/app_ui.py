...
class DatabaseManager:
    ...
    def create_table(self):
        with self.conn:
            self.conn.execute("""CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY, name TEXT NOT NULL UNIQUE);""")
            self.conn.execute("""CREATE TABLE IF NOT EXISTS face_encodings (id INTEGER PRIMARY KEY, user_id INTEGER, encoding BLOB NOT NULL, FOREIGN KEY (user_id) REFERENCES users (id));""")
    ...
    def get_known_face_encodings(self):
        ...
        with self.conn:
            cursor = self.conn.cursor()
            cursor.execute("SELECT u.name, f.encoding FROM users u JOIN face_encodings f ON u.id = f.user_id;")
...
class ModelLoader(QThread):
    ...
    def __init__(self, db_manager):
        super().__init__(); self.db_manager = db_manager
    def run(self):
...
class GestureController(QThread):
    frame_ready = pyqtSignal(np.ndarray); gesture_detected = pyqtSignal(str); user_recognized = pyqtSignal(str)
    def __init__(self):
        super().__init__(); self.running = False; self.camera_index = 0
    ...
    def run(self):
        ...
        if not self.known_face_encodings: print("Warning: No registered users found.")
        ...
        while self.running and cap.isOpened():
            ret, frame = cap.read()
            if not ret: break
            frame_counter += 1; frame = cv2.flip(frame, 1)
            ...
            if is_authorized_user:
                ...
                if result.multi_hand_landmarks:
                    for idx, hand_landmarks in enumerate(result.multi_hand_landmarks):
                        ...
                        if not result.multi_handedness or len(result.multi_handedness) <= idx: continue
                        handedness_label = result.multi_handedness[idx].classification[0].label
                        landmarks = hand_landmarks.landmark
                        if gesture_counter == 0:
                            if handedness_label == 'Right': # Physical Right Hand
                                ...
                                fingers_curled = (middle_tip_y > middle_pip_y and ring_tip_y > ring_pip_y and pinky_tip_y > pinky_pip_y)
                                index_extended = index_tip_y < index_pip_y
                                is_pointing = fingers_curled and index_extended
                                if is_pointing:
                                    index_tip_x = landmarks[mp_hands.HandLandmark.INDEX_FINGER_TIP].x
                                    wrist_x = landmarks[mp_hands.HandLandmark.WRIST].x
                                    if index_tip_x > wrist_x: # Pointing right on screen
                                        self.gesture_detected.emit("Next"); pyautogui.press('right'); gesture_counter = COOLDOWN_FRAMES
                                    elif index_tip_x < wrist_x: # Pointing left on screen
                                        self.gesture_detected.emit("Previous"); pyautogui.press('left'); gesture_counter = COOLDOWN_FRAMES
                            elif handedness_label == 'Left': # Physical Left Hand
                                ...
                                if pinch_distance > ZOOM_IN_THRESHOLD:
                                    self.gesture_detected.emit("Zoom In"); pyautogui.hotkey('ctrl', '+'); gesture_counter = COOLDOWN_FRAMES
                                elif pinch_distance < ZOOM_OUT_THRESHOLD:
                                    self.gesture_detected.emit("Zoom Out"); pyautogui.hotkey('ctrl', '-'); gesture_counter = COOLDOWN_FRAMES
...
class GestureGuideDialog(QDialog):
    def __init__(self, parent=None):
        ...
        right_hand_layout.addWidget(self.create_gesture_widget("next.png", "Point Right (next slide)"))
        right_hand_layout.addWidget(self.create_gesture_widget("previous.png", "Point Left (previous slide)"))
        ...
        left_hand_layout.addWidget(self.create_gesture_widget("zoom_in.png", "Pinch Apart (zoom in)"))
        left_hand_layout.addWidget(self.create_gesture_widget("zoom_out.png", "Pinch Together (zoom out)"))
    ...
    def create_gesture_widget(self, image_path, text):
        ...
        # <<< FIX: Use the resource_path function to find images >>>
        try: source_pixmap = QPixmap(resource_path(image_path))
...
class PresentationApp(QMainWindow):
    def __init__(self):
        ...
        self.db_manager = DatabaseManager(db_name=resource_path("user_data.db"))
