# notapro.py (versão com grifo e sublinhado)
...
class NotaPro(tk.Tk):
    ...
    def _create_menu(self):
        ...
        # --- NOVOS ITENS DE MENU ---
        format_menu.add_command(label="Grifar...", command=self._highlight_selection)
        format_menu.add_command(label="Sublinhado", accelerator="Ctrl+U", command=self._toggle_underline)
        format_menu.add_separator()
    ...
    def _save_npro_file(self, filepath):
        ...
        for tag_name in self.text_widget.tag_names():
            if tag_name.startswith("style_"):
                ...
                if config: # Apenas salvar se houver alguma configuração
    ...
    def _highlight_selection(self):
        if self.text_widget.tag_ranges(tk.SEL):
            color = colorchooser.askcolor(title="Escolha a cor do grifo")
            if color and color[1]:
                tag_name = f"style_{self.style_counter}"; self.style_counter += 1
                self.text_widget.tag_add(tag_name, "sel.first", "sel.last")
                self.text_widget.tag_config(tag_name, background=color[1])
                self._on_text_change(None)
        else: messagebox.showinfo("Nenhuma Seleção", "Selecione um trecho de texto para grifar.")
