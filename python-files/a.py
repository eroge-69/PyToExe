...
# --------------------------- Config ---------------------------------
DEFAULT_TOP_N = 20           # en çok işlem gören kaç hisseyi tarayalım
...
def check_volume_spike(symbol):
    ...
    try:
        # 1 dakikalık interval için period mutlaka "1d" olmalı
        df = yf.download(symbol, period="1d", interval=INTERVAL, progress=False, threads=False)
...
def main():
    ...
    while True:
        start = time.time()
        tickers = fetch_most_active(top_n=DEFAULT_TOP_N)
        if not tickers:
