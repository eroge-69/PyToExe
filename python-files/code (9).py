# notapro.py (versão com grifo opaco e sublinhado corrigido)
...
class NotaPro(tk.Tk):
    ...
    def _create_menu(self):
        ...
        # --- NOMES DE MENU ATUALIZADOS ---
        format_menu.add_command(label="Grifar trecho...", command=self._highlight_selection)
        format_menu.add_command(label="Sublinhar trecho", accelerator="Ctrl+U", command=self._toggle_underline)
        format_menu.add_separator()
    ...
    def _bind_shortcuts(self):
        ...
        self.bind("<Control-plus>", lambda e: self._zoom(10))
        self.bind("<Control-minus>", lambda e: self._zoom(-10))
        self.bind("<Control-0>", lambda e: self._zoom(0))
        self.bind("<Control-y>", lambda e: self.text_widget.edit_redo())
        self.bind("<Control-u>", lambda e: self._toggle_underline())
        self.text_widget.bind("<MouseWheel>", self._zoom_on_scroll)
        
    # --- FUNÇÃO AUXILIAR PARA OPACIDADE ---
    def _blend_colors(self, fg_hex, bg_hex, alpha):
        """Mistura uma cor de primeiro plano (fg) com uma de fundo (bg) usando um fator alpha."""
        fg_rgb = self.winfo_rgb(fg_hex)
        bg_rgb = self.winfo_rgb(bg_hex)

        # A função winfo_rgb retorna valores de 16 bits (0-65535), normalizamos para 0-255
        fg_r, fg_g, fg_b = [x / 256 for x in fg_rgb]
        bg_r, bg_g, bg_b = [x / 256 for x in bg_rgb]

        # Fórmula de mistura alpha
        res_r = int(fg_r * alpha + bg_r * (1 - alpha))
        res_g = int(fg_g * alpha + bg_g * (1 - alpha))
        res_b = int(fg_b * alpha + bg_b * (1 - alpha))
    ...
    def _save_npro_file(self, filepath):
        ...
        for tag_name in self.text_widget.tag_names():
            if tag_name.startswith("style_"):
                config = {}
                fg = self.text_widget.tag_cget(tag_name, "foreground")
                bg = self.text_widget.tag_cget(tag_name, "background")
                ul = self.text_widget.tag_cget(tag_name, "underline")
                ...
                if config:
                    ...
                    for i in range(0, len(ranges), 2):
                        data["tags"].append({"name": tag_name, "start": str(ranges[i]), "end": str(ranges[i+1]), "config": config})
    ...
    def _highlight_selection(self):
        ...
        opacity = simpledialog.askinteger("Opacidade", "Digite a opacidade do grifo (1-100%):",
                                          parent=self, minvalue=1, maxvalue=100)
        if opacity is None: return

        main_bg_color = self.text_widget.cget("bg")
    ...
    def _toggle_underline(self):
        if self.text_widget.tag_ranges(tk.SEL):
            ...
            is_underlined = any(self.text_widget.tag_cget(tag, "underline") for tag in current_tags)
            
            tag_name = f"style_{self.style_counter}"; self.style_counter += 1
            self.text_widget.tag_add(tag_name, "sel.first", "sel.last")
            self.text_widget.tag_config(tag_name, underline=not is_underlined)
            self._on_text_change(None)
        else: messagebox.showinfo("Nenhuma Seleção", "Selecione um trecho de texto para sublinhar.")
