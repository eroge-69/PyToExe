# notapro.py (versão com remoção de grifo/sublinhado)
...
class NotaPro(tk.Tk):
    ...
    def _create_menu(self):
        ...
        # --- NOVOS ITENS DE MENU ---
        format_menu.add_command(label="Retirar grifo", command=self._remove_highlight)
        format_menu.add_command(label="Retirar sublinhado", command=self._remove_underline)
        format_menu.add_separator()
    ...
    def _new_window(self):
        try:
            command = [sys.executable] if getattr(sys, 'frozen', False) else [os.path.join(os.path.dirname(sys.executable), 'pythonw.exe') if os.path.exists(os.path.join(os.path.dirname(sys.executable), 'pythonw.exe')) else sys.executable, os.path.abspath(__file__)]
            subprocess.Popen(command)
        except Exception as e: messagebox.showerror("Erro ao Abrir Nova Janela", f"Não foi possível iniciar uma nova instância:\n{e}")

    # ... (o restante do código permanece o mesmo, incluindo as funções de formatação) ...
    ...
    def _save_npro_file(self, filepath):
        ...
        for tag_name in self.text_widget.tag_names():
            if tag_name.startswith("style_"):
                ...
                if ul is not None and ul != '': config["underline"] = bool(ul)
                if config:
    ...
    # --- NOVAS FUNÇÕES DE REMOÇÃO ---
    def _remove_highlight(self):
        """Retira o grifo (background) do trecho selecionado."""
        if not self.text_widget.tag_ranges(tk.SEL):
            messagebox.showinfo("Nenhuma Seleção", "Selecione um trecho para retirar o grifo.")
            return
        tag_name = f"style_{self.style_counter}"; self.style_counter += 1
        self.text_widget.tag_add(tag_name, "sel.first", "sel.last")
        self.text_widget.tag_config(tag_name, background="") # Reseta para o padrão do widget
        self._on_text_change(None)
        
    def _remove_underline(self):
        """Retira o sublinhado do trecho selecionado."""
        if not self.text_widget.tag_ranges(tk.SEL):
            messagebox.showinfo("Nenhuma Seleção", "Selecione um trecho para retirar o sublinhado.")
            return
        tag_name = f"style_{self.style_counter}"; self.style_counter += 1
        self.text_widget.tag_add(tag_name, "sel.first", "sel.last")
        self.text_widget.tag_config(tag_name, underline=False) # Desativa o sublinhado
        self._on_text_change(None)
