# notapro.py (versão com correção para PyInstaller)
...
import os
import sys
import subprocess # Importado para lançar um novo processo
...
class NotaPro(tk.Tk):
    ...
    def _new_window(self):
        """Lança uma nova instância da aplicação em um processo separado,
           lidando com a execução a partir do script .py ou do executável congelado.
        """
        try:
            command = []
            if getattr(sys, 'frozen', False):
                # Se estiver rodando como um executável (congelado pelo PyInstaller),
                # o comando é simplesmente o próprio executável.
                command = [sys.executable]
            else:
                # Se estiver rodando como um script .py (modo de desenvolvimento),
                # usamos o caminho do script e o interpretador Python.
                script_path = os.path.abspath(__file__)
                python_executable = sys.executable
                
                # Prefere 'pythonw.exe' para evitar a janela de console no Windows
            ...
            # Inicia o novo processo sem bloquear a janela atual
            subprocess.Popen(command)

        except Exception as e:
            messagebox.showerror("Erro ao Abrir Nova Janela", f"Não foi possível iniciar uma nova instância:\n{e}")
