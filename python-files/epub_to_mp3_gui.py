...
# Voice options: Display Name -> ElevenLabs Voice ID
VOICES = {
    "Your Custom Voice": "FQ8VmAXUrzoLcSCwRJC2",
    "British Sci-Fi Narrator": "ErXwobaYiN019PkySvjV"
}
...
def convert_epub_to_mp3(epub_path, output_dir, voice_id, status_label, progress):
    try:
...
def launch_gui():
    ...
    selected_voice = tk.StringVar(value=list(VOICES.keys())[0])
    ...
    def start_conversion():
        if not epub_path.get() or not output_path.get():
            ...
            return
        voice_id = VOICES[selected_voice.get()]
        threading.Thread(target=convert_epub_to_mp3, args=(epub_path.get(), output_path.get(), voice_id, status_label, progress), daemon=True).start()

    tk.Label(root, text="Select EPUB File:").pack(pady=5)
    tk.Entry(root, textvariable=epub_path, width=60).pack()
    ...
    tk.Label(root, text="Choose Voice:").pack(pady=5)
    voice_dropdown = ttk.Combobox(root, textvariable=selected_voice, values=list(VOICES.keys()), state="readonly", width=50)
    voice_dropdown.pack()

    tk.Button(root, text="Start Conversion", command=start_conversion).pack(pady=15)
