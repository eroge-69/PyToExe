...
def pick_header_row(file_path, file_type="csv", sheet_name=None):
    ...
    tk.Label(row_frame, text="Select which row contains headers:").pack(side="left", padx=5)
    row_spin = tk.Spinbox(row_frame, from_=0, to=len(raw_df)-1, textvariable=row_var, width=5)
    row_spin.pack(side="left")

    def highlight_row(*args):
        for i in tree.get_children():
            tree.item(i, tags=())
        selected_idx = min(max(0, row_var.get()), len(raw_df)-1)
        item_id = tree.get_children()[selected_idx]
        tree.item(item_id, tags=("header",))
...
def pick_sheet(file_path):
    ...
    tk.Button(sheet_window, text="Confirm", command=confirm).pack(pady=10)
...
def map_columns(bc_df, vendor_df):
    ...
    for i, (label, default_bc) in enumerate(fields):
        tk.Label(frame_map, text=f"{label}:", font=("Arial", 11)).grid(row=i, column=0, padx=10, pady=5, sticky="e")
...
# --- Get Unique Filename with date ---
def get_unique_filename(original_file, suffix="_Updated", ext=None):
    folder = os.path.dirname(original_file)
    ...
    while True:
        if i == 0:
            filename = f"{base_name}{suffix}_{date_str}{ext}"
        else:
            filename = f"{base_name}{suffix}_{date_str}_{i}{ext}"
...
def process_files():
    ...
    try:
        ...
        # normalize text + strip leading zeros
        def clean_str(s):
            return str(s).strip().lower().lstrip("0")

        for idx, row in bc_df.iterrows():
            sku = clean_str(row[sku_bc])
            match = vendor_df[vendor_df[sku_vendor].apply(clean_str) == sku]
            if match.empty:
        ...
        out_file = get_unique_filename(bc_file, suffix="_Updated")
        bc_df.to_csv(out_file, index=False)

        # Save SKUs not found in vendor (Excel)
        skipped_file = None
        if skipped_bc:
            skipped_bc_df = pd.DataFrame(skipped_bc)
            skipped_file = get_unique_filename(bc_file, suffix="_Not_in_Excel")
            skipped_bc_df.to_csv(skipped_file, index=False)

        # Save Vendor items not in BigCommerce (CSV)
        unmatched_vendor = vendor_df[~vendor_df[sku_vendor].apply(clean_str)
                                     .isin(bc_df[sku_bc].apply(clean_str))]
        unmatched_file = None
        if not unmatched_vendor.empty:
            unmatched_file = get_unique_filename(bc_file, suffix="_Not_in_CSV")
...
tk.Label(root, text="BigCommerce Vendor Updater", font=("Arial", 14, "bold")).pack(pady=20)
