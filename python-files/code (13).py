# notapro.py (versão com menu de contexto)
...
class NotaPro(tk.Tk):
    def __init__(self):
        ...
        self._create_menu()
        self._create_context_menu() # --- NOVO: Chama a criação do menu de contexto
    ...
    def _create_widgets(self):
        ...
        # --- NOVO: Vincula o clique direito ao método que exibe o menu ---
        self.text_widget.bind("<Button-3>", self._show_context_menu)
        
        self._update_status_bar()

    # --- NOVO: Método para criar o menu de contexto ---
    def _create_context_menu(self):
        ...
        self.context_menu.add_command(label="Selecionar tudo", command=self._select_all)

    # --- NOVO: Método que é chamado no clique direito para exibir o menu ---
    def _show_context_menu(self, event):
        # Habilita/desabilita as opções com base no contexto
        ...
        try:
            self.clipboard_get()
            self.context_menu.entryconfig("Colar", state=tk.NORMAL)
        except tk.TclError:
            self.context_menu.entryconfig("Colar", state=tk.DISABLED)
    ...
    def _create_menu(self):
        # ... (O código do menu principal permanece inalterado) ...
        ...
        file_menu.add_command(label="Novo", accelerator="Ctrl+N", command=self._new_file); file_menu.add_command(label="Abrir...", accelerator="Ctrl+O", command=self._open_file)
        file_menu.add_command(label="Salvar", accelerator="Ctrl+S", command=self._save_file); file_menu.add_command(label="Salvar Como...", command=self._save_as_file)
    ...
    def _bind_shortcuts(self):
        # ... (O código dos atalhos permanece inalterado) ...
