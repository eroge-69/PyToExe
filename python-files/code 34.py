# notapro.py (versão com copiar/colar de formatação)
...
class NotaPro(tk.Tk):
    def __init__(self):
        ...
        # --- NOVO: Área de transferência interna para formatação ---
    ...
    def _create_context_menu(self):
        ...
        # --- ALTERAÇÃO: Chama as novas funções ricas ---
        self.context_menu.add_command(label="Recortar", command=self._cut_text)
        self.context_menu.add_command(label="Copiar", command=self._copy_rich_text)
        self.context_menu.add_command(label="Colar", command=self._paste_rich_text)
    ...
    def _show_context_menu(self, event):
        ...
        # Habilita o colar se tiver algo na área de transferência interna OU do sistema
        can_paste = bool(self.internal_clipboard)
        if not can_paste:
            try: can_paste = bool(self.clipboard_get())
            except tk.TclError: pass
        self.context_menu.entryconfig("Colar", state=tk.NORMAL if can_paste else tk.DISABLED)
    ...
    def _create_menu(self):
        ...
        edit_menu.add_command(label="Copiar", accelerator="Ctrl+C", command=self._copy_rich_text)
        edit_menu.add_command(label="Colar", accelerator="Ctrl+V", command=self._paste_rich_text)
    ...
    def _bind_shortcuts(self):
        ...
        # --- ALTERAÇÃO: Chama as novas funções ricas ---
        self.bind("<Control-x>", lambda e: self._cut_text())
        self.bind("<Control-c>", lambda e: self._copy_rich_text())
        self.bind("<Control-v>", lambda e: self._paste_rich_text())
        ...
        return "break" # Impede que os atalhos padrão sejam executados após os nossos

    # --- FUNÇÕES ATUALIZADAS E NOVAS PARA COPIAR/COLAR COM FORMATAÇÃO ---
    def _cut_text(self):
        if self._get_all_selections():
            self._copy_rich_text()
            # Deleta apenas a seleção padrão, pois recortar multi-seleções é complexo
    ...
    def _copy_rich_text(self):
        ...
        # Pega a primeira seleção para o dump
        start, end = ranges[0]

        content_dump = self.text_widget.dump(start, end, tag=True, text=True)
        tag_configs = {}
        for tag in self.text_widget.tag_names():
            if tag.startswith("style_"):
                tag_configs[tag] = self.text_widget.tag_config(tag)
        
        clipboard_data = {
            "dump": content_dump,
            "tags": tag_configs
        }
        self.internal_clipboard = json.dumps(clipboard_data)

        # Copia texto puro para o clipboard do sistema
        plain_text = self.text_widget.get(start, end)
        self.clipboard_clear()
        self.clipboard_append(plain_text)

    def _paste_rich_text(self):
        if self.internal_clipboard:
            try:
                # Se tivermos formatação, usa a nossa lógica
                clipboard_data = json.loads(self.internal_clipboard)
                content_dump = clipboard_data["dump"]
                tag_configs = clipboard_data["tags"]

                # Remove a seleção atual antes de colar
                if self.text_widget.tag_ranges(tk.SEL):
                    self.text_widget.delete(tk.SEL_FIRST, tk.SEL_LAST)

                insert_point = self.text_widget.index(tk.INSERT)
                
                # Mapa para evitar recriar tags com a mesma configuração
                original_to_new_tag = {}

                for key, value, index in content_dump:
                    if key == "tagon":
                        if value not in original_to_new_tag:
                            new_tag_name = f"style_{self.style_counter}"
                            self.style_counter += 1
                            original_to_new_tag[value] = new_tag_name
                            
                            # Recria a tag com a configuração salva
                            if value in tag_configs:
                                self.text_widget.tag_configure(new_tag_name, **tag_configs[value])
                        
                        self.text_widget.mark_set(f"{value}_start", tk.INSERT)
                    elif key == "tagoff":
                        if value in original_to_new_tag:
                            new_tag = original_to_new_tag[value]
                            start_mark = f"{value}_start"
                            self.text_widget.tag_add(new_tag, start_mark, tk.INSERT)
                            self.text_widget.mark_unset(start_mark)
                    elif key == "text":
                        self.text_widget.insert(tk.INSERT, value)
            except Exception as e:
                print(f"Erro ao colar texto formatado: {e}")
                self.text_widget.event_generate("<<Paste>>") # Fallback para colar texto puro
        else:
            # Se não tiver, usa a colagem padrão do sistema
            self.text_widget.event_generate("<<Paste>>")

    # ... (restante do código permanece igual, omitido por brevidade)
