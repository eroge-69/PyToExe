# main_clean.py
"""
NEM / Solar / Failures Console App
- All outputs are printed at the end
- Cleaned & organized for readability and maintenance
...
def ask_nonneg_int(prompt):
    """Ask for a nonnegative integer; keeps asking until valid."""
    while True:
...
def collect_items(count_prompt, item_label):
    """Collect a list of strings of length provided by the user."""
    items = []
...
def print_numbered(items):
    """Print a simple numbered list from any iterable of strings."""
    for idx, txt in enumerate(items, start=1):
        print(f"{idx}. {txt}")


# ---------------------------- PRINTERS (END-ONLY OUTPUT) ---------------------------- #

def print_user_inputs(nem_address, xfmr_id, feeder_number, feeder_name, voltage):
    print("\n--- User Input Summary ---")
...
def print_solar_summary(solar_results):
...
def print_case_summaries(final_lines):
    # BEFORE case
...
def print_field_team_notes(data_store):
    # BEFORE case
    ...
    if data_store["before"]["service_drop"]:
        ...
        print_numbered([
            f"Field team needs to sketch/investigate service drop at facility ID: {d['facility_id']}"
            for d in data_store["before"]["service_drop"]
        ])
    else:
    ...
    if data_store["before"]["xfmr"]:
        ...
        print("\nNotes for xfmrs:")
        print_numbered([
            f"Please sketch manhole or pole for existing xfmr at facility ID: {x['facility_id']}"
            for x in data_store["before"]["xfmr"]
        ])
    else:
    ...
    if data_store["after"]["service_drop"]:
        ...
        print_numbered([
            f"Field team needs to sketch/investigate service drop at facility ID: {d['facility_id']}"
            for d in data_store["after"]["service_drop"]
        ])
    else:
    ...
    if data_store["after"]["xfmr"]:
        ...
        print_numbered([
            f"Please sketch manhole or pole for existing xfmr at facility ID: {x['facility_id']}"
            for x in data_store["after"]["xfmr"]
        ])
    else:
...
def main():
    ...
    if not ask_yes_no("Is there any existing solar?"):
        ...
        if existed_before:
            ...
            if len(existing_before) == 0:
                ...
                solar_results.append(
                    f"{len(existing_before)} existing solars: {before_str} prior to 1/1/24. Solar power factors set to 1"
                )
        else:
        ...
        if existed_after:
            ...
            if len(existing_after) == 0:
                ...
                solar_results.append(
                    f"{len(existing_after)} existing solars: {after_str} on or after 1/1/24. Solar power factors set to 1"
                )
        else:
            solar_results.append("No existing solar after 1/1/2024")
        ...
        if not has_pending:
            ...
            if len(pending) == 0:
                ...
                solar_results.append(
                    f"{len(pending)} pending solars: {pending_str}. Solar power factors set to 1"
                )
    ...
    print_user_inputs(nem_address, xfmr_id, feeder_number, feeder_name, voltage)
    print_solar_summary(solar_results)
    print_case_summaries(final_lines)
    print_field_team_notes(data_store)
