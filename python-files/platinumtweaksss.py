...
# Funkce pro aktualizaci CPU a RAM
def update_vitals():
...
pc_info = tk.Label(dashboard, text=f"CPU Frequency: {psutil.cpu_freq().current:.0f} MHz\n"
                                   f"RAM Total: {round(psutil.virtual_memory().total / (1024**3), 2)} GB",
                   bg="#ecf0f1")
pc_info.pack(pady=10)

# Pravý panel - Live Vitals a Core Status
