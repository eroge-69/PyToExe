# notapro.py (versão com a funcionalidade Localizar)
...
# --- NOVA JANELA DE DIÁLOGO PARA LOCALIZAR ---
class FindDialog(tk.Toplevel):
    def __init__(self, master):
        ...
        tk.Label(main_frame, text="Localizar:").grid(row=0, column=0, sticky='w')
        self.find_entry = tk.Entry(main_frame)
        self.find_entry.grid(row=0, column=1, columnspan=2, sticky='we', padx=(5, 0))
        self.find_entry.focus_set()
        # Preenche com o último termo buscado
        self.find_entry.insert(0, self.master.last_search_term)

        self.find_button = tk.Button(main_frame, text="Localizar próxima", width=15, command=self._find_next)
        self.find_button.grid(row=0, column=3, padx=(10, 0))
        self.cancel_button = tk.Button(main_frame, text="Cancelar", width=15, command=self.destroy)
        self.cancel_button.grid(row=1, column=3, padx=(10, 0), pady=5)
        
        # Opções de busca
        options_frame = tk.Frame(main_frame, borderwidth=1, relief=tk.GROOVE)
        options_frame.grid(row=1, column=0, columnspan=3, pady=5, sticky='we')

        self.match_case_check = tk.Checkbutton(options_frame, text="Diferenciar maiúsculas de minúsculas", variable=self.master.last_search_match_case)
        self.match_case_check.pack(side=tk.LEFT, padx=5)

        direction_frame = tk.Frame(main_frame)
        direction_frame.grid(row=2, column=1, columnspan=2, sticky='w')
        tk.Label(direction_frame, text="Direção:").pack(side=tk.LEFT, padx=5)
        tk.Radiobutton(direction_frame, text="Para cima", variable=self.master.last_search_direction, value="up").pack(side=tk.LEFT)
        tk.Radiobutton(direction_frame, text="Para baixo", variable=self.master.last_search_direction, value="down").pack(side=tk.LEFT)

        # Garante que o Enter no campo de texto também busca
        self.find_entry.bind('<Return>', lambda e: self._find_next())

    def _find_next(self):
        """Atualiza o estado da busca e chama a função principal de busca."""
        # Atualiza o termo de busca na janela principal
        self.master.last_search_term = self.find_entry.get()
        # Chama a função de busca real (a mesma usada pelo F3)
        self.master._find_next_f3()
...
class NotaPro(tk.Tk):
    def __init__(self):
        ...
        self.last_search_match_case = tk.BooleanVar(value=False)
        self.last_search_direction = tk.StringVar(value="down")
        self.find_dialog = None # Para evitar janelas duplicadas
    ...
    def _create_menu(self):
        ...
        edit_menu.add_command(label="Localizar próxima", accelerator="F3", command=self._find_next_f3)
        edit_menu.add_separator()
    ...
    def _open_find_dialog(self):
        """Abre a janela de diálogo para localizar texto."""
        # Evita abrir múltiplas janelas de busca
        if self.find_dialog is None or not self.find_dialog.winfo_exists():
            self.find_dialog = FindDialog(self)
        self.find_dialog.deiconify() # Mostra a janela se estiver minimizada
        self.find_dialog.lift() # Traz para frente
        self.find_dialog.focus_set()

    def _find_next_f3(self):
        """Executa a busca com base no estado salvo (usado pelo F3 e pela dialog)."""
        if not self.last_search_term:
            self._open_find_dialog()
            return
        self._perform_search(self.last_search_term, self.last_search_match_case.get(), self.last_search_direction.get())
        
    def _perform_search(self, term, match_case, direction):
        """A lógica central de busca no widget de texto."""
        ...
        pos = text_widget.search(term, start_pos, stopindex=end_pos, backwards=backwards, nocase=not match_case)

        if pos:
            end_char_pos = f"{pos}+{len(term)}c"
            text_widget.tag_add(tk.SEL, pos, end_char_pos)
            text_widget.mark_set(tk.INSERT, end_char_pos if direction == 'down' else pos)
            text_widget.see(pos)
        else:
