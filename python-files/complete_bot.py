...
REQUIRED_PACKAGES = [
    'flask', 'flask_socketio', 'cv2', 'pyautogui', 'PIL', 'numpy',
    'sqlalchemy', 'werkzeug'
]
...
def check_dependencies():
    ...
    if missing:
        ...
        print(
            "pip install flask flask-socketio opencv-python pyautogui pillow numpy sqlalchemy werkzeug"
        )
    ...
    def add_trade(self, symbol, trade_type, volume, open_price):
        ...
        return self.execute(
            """INSERT INTO trades (symbol, trade_type, volume, open_price, open_time) 
               VALUES (?, ?, ?, ?, ?)""",
            (symbol, trade_type, volume, open_price,
...
class SignalDetector:
        ...
        for signal in signals:
            ...
            for existing in filtered:
                if (signal['type'] == existing['type']
                        and abs(signal['position'][0] -
                                existing['position'][0]) < 50
                        and abs(signal['position'][1] -
                                existing['position'][1]) < 50):
                    is_duplicate = True
                    break
    ...
    def _process_signal(self, signal, screenshot):
        ...
        try:
            ...
            signal_id = self.db.add_signal(signal_type=signal_type,
                                           symbol='XAUUSD',
            ...
            logger.info(
                f"تم كشف إشارة: {signal_type} (الثقة: {signal['confidence']:.2f})"
            )
...
class MT5Connector:
    ...
    def connect(self):
        ...
        try:
            ...
            self.account_info[
                'equity'] = self.account_info['balance'] + random.uniform(
                    -100, 100)

            logger.info(
                f"تم الاتصال بـ MT5 - الحساب: {self.account_info['login']}")
    ...
    def get_account_info(self):
        ...
        self.account_info[
            'equity'] = self.account_info['balance'] + random.uniform(-50, 50)
    ...
    def execute_trade(self, symbol, trade_type, volume=None):
        ...
        try:
            ...
            trade_id = self.db.add_trade(symbol=symbol,
            ...
            logger.info(
                f"تم تنفيذ الصفقة: {trade_type} {volume} {symbol} بسعر {price}"
            )
    ...
    def _check_new_signals(self):
        ...
        try:
            ...
            for signal in signals:
                ...
                execute_time = datetime.fromisoformat(timestamp) + timedelta(
                    seconds=self.settings['signal_delay'])
                ...
                # تحديد كمنفذة
                self.db.execute("UPDATE signals SET executed = 1 WHERE id = ?",
                                (signal_id, ))
    ...
    def _execute_signal(self, signal):
        ...
        try:
            if not self.settings['auto_trade']:
                self.db.add_log(
                    'INFO',
                    f'التداول الآلي معطل، تجاهل الإشارة: {signal["type"]}')
                return
    ...
    def _execute_trade_signal(self, signal_type, symbol):
        ...
        try:
            ...
            if len(symbol_positions) >= self.settings['max_positions']:
                self.db.add_log(
                    'WARNING', f'تم الوصول للحد الأقصى من المراكز لـ {symbol}')
                return
            ...
            if result:
                self.db.add_log('INFO',
                                f'تم تنفيذ الصفقة: {signal_type} {symbol}')
                self.socketio.emit(
                    'trade_notification', {
                        'type': 'success',
                ...
                self.db.add_log(
                    'ERROR', f'فشل في تنفيذ الصفقة: {signal_type} {symbol}')
    ...
    def _execute_close_signal(self, signal_type, symbol):
        ...
        try:
            ...
            if success:
                self.db.add_log(
                    'INFO',
                    f'تم إغلاق المراكز بسبب إشارة {signal_type} لـ {symbol}')
                self.socketio.emit(
                    'trade_notification', {
                        'type': 'info',
    ...
    def start_bot():
        try:
            ...
            return jsonify({
                ...
                'message':
                'تم تشغيل البوت' if success else 'فشل في تشغيل البوت'
            })
        except Exception as e:
        ...
        if request.method == 'GET':
            ...
            try:
                ...
                return jsonify({
                    ...
                    'message':
                    'تم تحديث الإعدادات'
                    if success else 'فشل في تحديث الإعدادات'
                })
            except Exception as e:
                return jsonify({'status': 'error', 'message': str(e)})
