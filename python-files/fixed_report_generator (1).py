...
def prepare_report(symbols, dfs, today_df, today_fo_symbols, nifty500_symbols, nifty_close_map):
    # Use last 25 days for stats
    prev_df = pd.concat(dfs[:-1]) if len(dfs) > 1 else pd.DataFrame()
    today_df = dfs[-1]
    # --- corrected previous-14-days window (exclude today) ---
    if len(dfs) >= 15:
        prev_14_df = pd.concat(dfs[-15:-1])   # last 14 trading days BEFORE today
    else:
        prev_14_df = pd.concat(dfs[:-1]) if len(dfs) > 1 else pd.DataFrame()

    # compute avg_qnty_per_trade on the previous-window only (avoid dividing by zero)
    if not prev_14_df.empty and 'TTL_TRD_QNTY' in prev_14_df.columns and 'NO_OF_TRADES' in prev_14_df.columns:
        prev_14_df = prev_14_df.copy()
    ...
    # Prepare a merged DataFrame for all days, sorted by SYMBOL and DATE
    ...
    # Calculate price change and delivery change using groupby and shift
    ...
    for symbol in symbols:
        ...
        # Use 14-day median for qnty_per_trade and avg_qnty_per_trade from previous-window (exclude today)
        prev_14_df_symbol = prev_14_df[prev_14_df["SYMBOL"] == symbol] if not prev_14_df.empty else pd.DataFrame()
        if not prev_14_df_symbol.empty and 'avg_qnty_per_trade' in prev_14_df_symbol.columns:
            # previous day's avg_qnty_per_trade (most recent value in prev window)
            qnty_per_trade_val = prev_14_df_symbol['avg_qnty_per_trade'].iloc[-1]
            avg_qnty_per_trade_val = prev_14_df_symbol['avg_qnty_per_trade'].median()
        else:
            qnty_per_trade_val = np.nan
            avg_qnty_per_trade_val = np.nan

        deliv_qty = float(today_row.get("DELIV_QTY", np.nan)) if 'DELIV_QTY' in today_row else np.nan
        avg_deliv_qty = prev_14_df_symbol['DELIV_QTY'].median() if not prev_14_df_symbol.empty and 'DELIV_QTY' in prev_14_df_symbol.columns else np.nan

        # Calculate percentage deviation for trade quantity (compared to mean from prev_df)
        if symbol in means.index and 'TTL_TRD_QNTY' in means.columns and means.loc[symbol, 'TTL_TRD_QNTY'] != 0:
            trade_qty_deviation = ((float(today_row.get("TTL_TRD_QNTY", np.nan)) - means.loc[symbol, "TTL_TRD_QNTY"]) / means.loc[symbol, "TTL_TRD_QNTY"]) * 100
        else:
            trade_qty_deviation = np.nan

        # Calculate percentage deviation for delivery quantity against median over previous window
        ...
        fo_flag = 'Y' if symbol in today_fo_symbols else 'N'
        nifty500_flag = 'Y' if symbol in nifty500_symbols else 'N'

        # Use last 20 rows for delivery averages and vwap stats
        symbol_df = all_df[all_df["SYMBOL"] == symbol].tail(20)
        symbol_df = symbol_df.copy()
        symbol_df["DATE"] = symbol_df["DATE"].dt.strftime("%d%m%Y")

        prev_14_df_symbol_1 = prev_14_df_symbol[:-1] if len(prev_14_df_symbol) > 1 else prev_14_df_symbol
        vwap_gt_close_7 = int((prev_14_df_symbol_1.tail(7)["AVG_PRICE"] < prev_14_df_symbol_1.tail(7)["CLOSE_PRICE"]).sum()) if not prev_14_df_symbol_1.empty else 0
        vwap_lt_close_7 = int((prev_14_df_symbol_1.tail(7)["AVG_PRICE"] > prev_14_df_symbol_1.tail(7)["CLOSE_PRICE"]).sum()) if not prev_14_df_symbol_1.empty else 0
        vwap_gt_close_14 = int((prev_14_df_symbol_1.tail(14)["AVG_PRICE"] < prev_14_df_symbol_1.tail(14)["CLOSE_PRICE"]).sum()) if not prev_14_df_symbol_1.empty else 0
        vwap_lt_close_14 = int((prev_14_df_symbol_1.tail(14)["AVG_PRICE"] > prev_14_df_symbol_1.tail(14)["CLOSE_PRICE"]).sum()) if not prev_14_df_symbol_1.empty else 0
        ...
        # --- RS ROC calculation with date cross-check ---
        symbol_dates = prev_14_df_symbol["DATE"].tolist() if not prev_14_df_symbol.empty else []
        asset_closes = prev_14_df_symbol["CLOSE_PRICE"].values if 'CLOSE_PRICE' in prev_14_df_symbol.columns else np.array([])
        valid_pairs = [(ac, d) for ac, d in zip(asset_closes, symbol_dates) if d in nifty_close_map]
        if len(valid_pairs) >= 14:
            ...
            rs_df = pd.DataFrame({
                "asset_close": asset_closes_aligned,
                "nifty_close": nifty_closes_aligned
            })
        ...
        # Build report row with requested column names and ordering.
        # Place ROC_RS_7, ROC_RS_14, ROC_RS_7_14_Diff right after the 8th column (after delivery_qty_deviation)
        report_rows.append({
            "Symbol": symbol,
            "Date": datetime.today().strftime("%Y%m%d"),
            "Qty_ Per_Trade": qnty_per_trade_val,
            "Median_Qty_Per_Traade": avg_qnty_per_trade_val,
            "deliv_qty": deliv_qty,
