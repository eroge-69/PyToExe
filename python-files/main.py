...
def from_iso(s: Optional[str]) -> Optional[QDate]:
    """Safely convert ISO string (YYYY-MM-DD) to QDate, return None if invalid."""
    if not s:
        return None
    try:
    ...
    def __init__(self, db: DB, student: Optional[sqlite3.Row] = None, parent=None):
        ...
        if student:
            ...
            if student["start_date"]:
                qd = from_iso(student["start_date"])
                if qd:
                    self.start.setDate(qd)
            if student["next_due_date"]:
                qd = from_iso(student["next_due_date"])
                if qd:
                    self.due.setDate(qd)
