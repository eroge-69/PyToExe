...
class CustomGrid:
    ...
    def create(self):
        ...
        self.v_scrollbar.grid(row=0, column=1, sticky="ns", padx=0, pady=0)
        
        self.h_scrollbar = ctk.CTkScrollbar(self.master, orientation="horizontal", command=self.canvas.xview)
        self.h_scrollbar.grid(row=1, column=0, sticky="ew", padx=0, pady=0)
        
        self.canvas.configure(yscrollcommand=self.v_scrollbar.set, xscrollcommand=self.h_scrollbar.set)
        
        # Frames
        self.header_frame = ctk.CTkFrame(self.canvas, fg_color="#F5F5F5", height=30, corner_radius=0)
        self.data_frame = ctk.CTkFrame(self.canvas, fg_color="white", corner_radius=0)
        
        self.canvas.create_window((0, 0), window=self.header_frame, anchor="nw")
        self.canvas.create_window((0, 30), window=self.data_frame, anchor="nw")

        # Create headers
        window_width = self.master.winfo_width() or 800
        num_cols = len(self.columns)
        col_width = max(90, window_width // num_cols)
        ...
        for col_idx, col_name in enumerate(reversed(self.columns)):
            ...
            header.grid(row=0, column=col_idx, padx=0, pady=0, sticky="nsew")
            self.header_frame.grid_columnconfigure(col_idx, weight=1, minsize=col_width)
    ...
    def _bind_mousewheel(self):
        ...
        if self.master.winfo_exists():
            self.master.bind("<MouseWheel>", self._on_mousewheel_vertical)
            self.master.bind("<Shift-MouseWheel>", self._on_mousewheel_horizontal)
    ...
    def _on_mousewheel_vertical(self, event):
        try:
            ...
            logging.debug(f"Mousewheel scroll error: {str(e)}")
    ...
    def _resize_columns(self, event=None):
        ...
        try:
            window_width = max(100, self.master.winfo_width() - 20)
            num_cols = len(self.columns)
            col_width = max(80, window_width // num_cols)
                ...
                for col_idx in range(num_cols):
                    ...
                    for child in self.header_frame.winfo_children():
                        if isinstance(child, ctk.CTkLabel):
                            child.configure(width=col_width)
            ...
            logging.debug(f"Resize error: {str(e)}")
    ...
    def update_data(self, data):
        ...
        # Calculate tight column widths
        window_width = self.master.winfo_width() or 800
        num_cols = len(self.columns)
        col_width = max(90, window_width // num_cols)

        # Create ultra-compact data rows
        for row_idx, row_data in enumerate(data):
            ...
            for col_idx, value in enumerate(reversed(row_data[1:])):  # Skip ID
                label = ctk.CTkLabel(
                    ...
                    text_color="#1A2526",
                    width=col_width,
                    height=24,  # Very compact row height
                    padx=2,    # Minimal internal padding
                    corner_radius=0
                )
                label.grid(
                    row=row_idx,
                    column=col_idx,
                    padx=0,    # Zero padding between columns
                    pady=0,    # Zero padding between rows
                    sticky="nsew"
                )
                ...
                self.data_frame.grid_columnconfigure(col_idx, weight=1, minsize=col_width)
...
class SalesSystem:
    ...
    def show_akharajat(self):
        ...
        for btn in self.menu_buttons:
            btn.configure(fg_color="#0078D7" if btn.cget("text") == "اخراجات" else "#E8ECEF", 
                        text_color="white" if btn.cget("text") == "اخراجات" else "#1A2526")

        try:
            # Create a container frame with proper padding
            container = ctk.CTkFrame(self.content_frame, fg_color="white")
            container.pack(fill="both", expand=True, padx=10, pady=10)
            
            # Call the akharajat main function with our container
            akharajat_main(container, self.urdu_font)
            
            # After the treeview is created, adjust its styling
            self.root.after(100, self._fix_akharajat_treeview)
    ...
    def _fix_akharajat_treeview(self):
        """Fix visibility issues in the akharajat treeview"""
        try:
            # Find all treeviews in the content frame
            for widget in self.content_frame.winfo_children():
                if hasattr(widget, 'winfo_children'):
                    for child in widget.winfo_children():
                        if 'Treeview' in str(child.__class__):
                            # Configure treeview styling
                            child.configure(
                                font=self.urdu_font,
                                foreground="#1A2526",  # Dark text color
                                background="white",    # White background
                                fieldbackground="white",
                                selectbackground="#0078D7",  # Blue selection
                            ...
                            for col in child['columns']:
                                child.column(col, width=150)  # Adjust width as needed
                                child.heading(col, text=col)
        except Exception as e:
            logging.error(f"Error fixing treeview: {str(e)}")
