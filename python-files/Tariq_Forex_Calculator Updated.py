...
class TariqForexCalculator:
    def __init__(self, root):
        ...
        self.create_label_entry("📦 Lot Size", "lot_size")  # Now editable
        self.create_label_value("🎯 Pips Needed to Win", "pips_needed", readonly=True)
        self.create_label_entry("📈 Risk/Reward", "rr_ratio")  # New editable box
    ...
    def calculate(self):
        try:
            ...
            # Lot size: if user entered manually, use it; otherwise calculate
            lot_size_text = self.lot_size.get()
            if lot_size_text.strip() == "":
            ...
            # Pips needed = target / (lot × pip_value)
            ...
            # Risk/Reward = Target / Risk $
            rr = target / risk_dollars if risk_dollars != 0 else 0
            self.rr_ratio.delete(0, tk.END)
            self.rr_ratio.insert(0, f"{rr:.2f}")

        except Exception as e:
            print("Error:", e)

    def clear_all(self):
        for var in ["account_size", "risk_percent", "risk_dollars", "target", "stop_size", "lot_size", "pips_needed", "rr_ratio"]:
