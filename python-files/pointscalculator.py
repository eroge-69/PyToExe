...
# Michael Uwagie-Ero is the G.O.A.T
    ...
    def load_files(self):
        ...
        try:
            ...
            for file in filepaths:
                ...
                df["FullName"] = (
                    df.iloc[:,0].astype(str).str.strip() + " " + df.iloc[:,1].astype(str).str.strip()
                )

                # Count each unique name only once per file
                unique_names = df["FullName"].dropna().unique()
